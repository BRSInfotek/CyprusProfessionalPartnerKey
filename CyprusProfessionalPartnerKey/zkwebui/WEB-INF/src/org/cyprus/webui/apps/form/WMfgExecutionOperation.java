...
import org.cyprus.webui.apps.MFGExecutionOperation;
 ...
 * 
 */
public class WMfgExecutionOperation extends MFGExecutionOperation implements IFormController, EventListener, ValueChangeListener
{
	private static WGenForm form;
		
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(WMfgExecutionOperation.class);
	...
	private Label lActivityFrom = new Label();
	private Label lActivityTo = new Label();
	private Label lOperation = new Label();
	private WSearchEditor fOperation;
	private Label lOperationFrom = new Label();
	private WSearchEditor fOperationFrom;
	private Label lOperationTo = new Label();
	private WSearchEditor fOperationTo;
	...
	private Label     lStepFrom = new Label();
	private WTableDirEditor  cmbStepFrom;
	private Label     lStepTo = new Label();
	private WTableDirEditor  cmbStepTo;
	...
	private Label lLocator = new Label();
	 private Listbox pickActivityFrom = new Listbox();
	 private Listbox pickActivityTo = new Listbox();
	 private Listbox pickLocator = new Listbox();
	 private Button Icon = new Button();
	
	public WMfgExecutionOperation()
	{
		log.info("");
	...
	void zkInit() throws Exception
	{
		...
		lStepFrom.setText("Step From");
		lStepTo.setText("Step To");
		lActivityFrom.setText("From Activity");
		lActivityTo.setText("To Activity");
		lOperationFrom.setText("From Operation");
		lOperationTo.setText("To Operation");
		lQty.setText(Msg.translate(Env.getCtx(), "Qty"));
		Icon.setLabel(Msg.translate(Env.getCtx(), "Record"));
		...
		row = new Row();
		form.getParameterPanel().getRows().appendChild(row);
		row.appendChild(lStepFrom.rightAlign());
		row.appendChild(cmbStepFrom.getComponent());
		row.appendChild(new Space());
		row.appendChild(lActivityFrom.rightAlign());
		row.appendChild(pickActivityFrom);
		row.appendChild(new Space());
		row.appendChild(lOperationFrom.rightAlign());
		row.appendChild(fOperationFrom.getComponent());
		row.appendChild(new Space());
		
		row = new Row();
		form.getParameterPanel().getRows().appendChild(row);
		row.appendChild(lStepTo.rightAlign());
		row.appendChild(cmbStepTo.getComponent());
		row.appendChild(new Space());
		row.appendChild(lActivityTo.rightAlign());
		row.appendChild(pickActivityTo);
		row.appendChild(new Space());
		row.appendChild(lOperationTo.rightAlign());
		row.appendChild(fOperationTo.getComponent());
		row.appendChild(new Space());
		...
		row.appendChild(new Space());
		row.appendChild(new Space());
		row.appendChild((Component)Icon);	
		row.appendChild(new Space());
	}	//	jbInit
	...
	{		
		...
		MLookup operationFromL = MLookupFactory.get (Env.getCtx(), form.getWindowNo(), 0, 1002341, DisplayType.Search);
		fOperationFrom = new WSearchEditor ("M_Operation_ID", false, true, true, operationFromL);
		fOperationFrom.addValueChangeListener(this);
		
		MLookup operationToL = MLookupFactory.get (Env.getCtx(), form.getWindowNo(), 0, 1002341, DisplayType.Search);
		fOperationTo = new WSearchEditor ("M_Operation_ID", false, true, true, operationToL);
		fOperationTo.addValueChangeListener(this);
		...
		pickActivityFrom.setMold("select");
		pickActivityFrom.setRows(0);
//		pickActivityFrom.setSelectedIndex(0);
		pickActivityFrom.addEventListener(Events.ON_SELECT, this);
		
		pickActivityTo.setMold("select");
		pickActivityTo.setRows(0);
//		pickActivityTo.setSelectedIndex(0);
		pickActivityTo.addEventListener(Events.ON_SELECT, this);
...
//		pickLocator.setSelectedIndex(0);
	    ...
	    MLookup trxTypeL = MLookupFactory.get(Env.getCtx(), form.getWindowNo(), 53827,
				...
				false, "AD_Ref_List.Value IN ('170')");
	    cmbTrxType = new WTableDirEditor("CostCollectorType", true, true, false,trxTypeL);
	    cmbTrxType.setValue("170");
	    cmbTrxType.addValueChangeListener(this);
	    
	    MLookup stepFromL = MLookupFactory.get(Env.getCtx(), form.getWindowNo(), 1002823,
				DisplayType.List, Env.getLanguage(Env.getCtx()), "StepFrom", 1000054,
				false, "AD_Ref_List.Value IN ('QU','IP','FH')"); //1000054
	    cmbStepFrom = new WTableDirEditor("StepFrom", true, false, true,stepFromL);
	    cmbStepFrom.addValueChangeListener(this);
	    
	    MLookup stepToL = MLookupFactory.get(Env.getCtx(), form.getWindowNo(), 1002824,
				DisplayType.List, Env.getLanguage(Env.getCtx()), "StepTo", 1000054,
				false, "AD_Ref_List.Value IN ('QU','IP','FH')"); // 1000054
	    cmbStepTo = new WTableDirEditor("StepTo", true, false, true,stepToL);
	    cmbStepTo.addValueChangeListener(this);
        ...
        form.getStatusBar().setStatusLine(Msg.getMsg(Env.getCtx(), "MFGExecutionOperation"));
	}	//	fillPicks
	...
	{
		if (e.getTarget() == pickActivityFrom)
		{
			KeyNamePair activityFromNamePair=null;
		
			try 
			{
				activityFromNamePair = pickActivityFrom.getSelectedItem().toKeyNamePair();
				if(activityFromNamePair!=null)
				{
					m_PP_Order_NodeFrom_ID = activityFromNamePair.getKey();
					int operationID = DB.getSQLValue(null,"SELECT M_Operation_ID FROM PP_Order_Node WHERE PP_Order_Node_ID=" + activityFromNamePair.getKey());
					fOperationFrom.setValue(operationID);
			        m_M_OperationFrom_ID = operationID;
				}
				else
				{
					fOperationFrom.setValue(null);
				     m_M_OperationFrom_ID = null;
				}
		...
		else if (e.getTarget() == pickActivityTo)
		{
			KeyNamePair activityToNamePair=null;
		
			try 
			{
				activityToNamePair = pickActivityTo.getSelectedItem().toKeyNamePair();
				if(activityToNamePair!=null)
				{
					m_PP_Order_NodeTo_ID = activityToNamePair.getKey();
					int operationID = DB.getSQLValue(null,"SELECT M_Operation_ID FROM PP_Order_Node WHERE PP_Order_Node_ID=" + activityToNamePair.getKey());
					fOperationTo.setValue(operationID);
			        m_M_OperationTo_ID = operationID;
				}
				else
				{
					fOperationTo.setValue(null);
				     m_M_OperationTo_ID = null;
				}
		...
		else if(e.getTarget()==Icon) {
//			WTimeRecording popup = new WTimeRecording();
//		    popup.setPage(form.getPage());
//		    popup.doHighlighted();
			...
			if(m_StepFrom == null)
			{
				throw new CyprusException("Please select Step From..");
			}
			if(m_StepTo == null)
			{
				throw new CyprusException("Please select Step To..");
			}
			if(m_PP_Order_NodeFrom_ID == null)
			{
				throw new CyprusException("Please select Activity From..");
			}
			if(m_PP_Order_NodeTo_ID == null)
			{
				throw new CyprusException("Please select Activity To..");
			}
			...
			WTimeRecording popup = new WTimeRecording();
			 popup.setPage(form.getPage());
		    popup.addEventListener("onTimeSave", (Event ev) -> {
		        timeRecords = (List<TimeRecordDTO>) ev.getData();
		        log.info("Received " + timeRecords.size() + " time records");
		    });
		    popup.doHighlighted();
		}
...
//			throw new CyprusException("Please select Manufacting Order..");
		...
		else if(m_StepFrom == null)
		{
//			throw new CyprusException("Please select Step From..");
			FDialog.warn(0, "Please select Step From..");
			return;
		}
		else if(m_StepTo == null)
		{
//			throw new CyprusException("Please select Step To..");
			FDialog.warn(0, "Please select Step To..");
			return;
		}
		else if(m_PP_Order_NodeFrom_ID == null)
		{
//			throw new CyprusException("Please select Activity From..");
			FDialog.warn(0, "Please select Activity From..");
			return;
		}
		else if(m_PP_Order_NodeTo_ID == null)
		{
//			throw new CyprusException("Please select Activity To..");
			FDialog.warn(0, "Please select Activity To..");
			return;
		}
...
//			throw new CyprusException("Please select Locator..");
	...
	public void valueChange(ValueChangeEvent e)
	{
		...
		if (e.getPropertyName().equals("PP_Order_ID")) {
	        ...
	        if (m_PP_Order_ID != null) {
	            ...
	            pickActivityFrom.removeAllItems();
	            pickActivityFrom.addItem (new KeyNamePair(0,s_none)) ;
	            pickActivityTo.removeAllItems();
	            pickActivityTo.addItem (new KeyNamePair(0,s_none)) ;
	    		...
	    		try
	    		{
	    			...
	    			while (rs.next ())
	    			{
	    				...
	    				if(nodeID > 0)
	    				{	
	    					pickActivityFrom.addItem(new KeyNamePair(nodeID, nodeName));
	    					pickActivityTo.addItem(new KeyNamePair(nodeID, nodeName));
	    				}
    				}
	    		}
	    		catch (Exception ex)
		 ...
		 if (e.getPropertyName().equals("StepFrom")) {
		        m_StepFrom = e.getNewValue();
	        }
		 
		 if (e.getPropertyName().equals("StepTo")) {
			 m_StepTo = e.getNewValue();
	        }
	...
	{		
		if (timeRecords == null || timeRecords.isEmpty()) {
	        throw new CyprusException("Please record time entries first..");
	    }	
