...
@IF '%CYPRUS_APPS_TYPE%' == 'tomcat' GOTO TOMCAT
@GOTO UNSUPPORTED
...
:TOMCAT
@Set NOPAUSE=Yes
@Set CATALINA_HOME=%CYPRUS_HOME%\tomcat
@Set JAVA_OPTS=-server %CYPRUS_JAVA_OPTIONS% %SECURE% -Dorg.cyprus.server.embedded=true

@Echo Start Tomcat Cyprus Apps Server %CYPRUS_HOME% (%CYPRUS_DB_NAME%)
@Call %CYPRUS_HOME%\tomcat\bin\startup.bat
@Echo Done Cyprus Apps Server %CYPRUS_HOME% (%CYPRUS_DB_NAME%)
@GOTO END
