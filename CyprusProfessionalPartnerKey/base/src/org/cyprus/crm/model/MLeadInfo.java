...
//		Get Defaults from Parent
		...
		if (getLine() == 0)
		{
			String sql = "SELECT COALESCE(MAX(Line),0)+10 FROM C_LeadInfo WHERE C_Lead_ID=?";
			int ii = DB.getSQLValue (get_TrxName(), sql, getC_Lead_ID());
			setLine (ii);
		}
...
//		m_M_PriceList_ID = lead.getM_PriceList_ID();
	...
	private boolean updateHeaderAmount()
	{
		...
		//Update Lead Header GrandTotal
				sql = "UPDATE C_Lead i"
					+ " SET GrandTotal="
						+ "(SELECT COALESCE(SUM(LineTotalAmount),0) FROM C_LeadInfo il WHERE i.C_Lead_ID=il.C_Lead_ID) "
					+ "WHERE C_Lead_ID=" + getC_Lead_ID();
