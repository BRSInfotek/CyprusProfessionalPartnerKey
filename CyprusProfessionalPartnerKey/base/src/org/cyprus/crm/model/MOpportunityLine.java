...
//		Get Line No
		if (getLine() == 0)
		{
			String sql = "SELECT COALESCE(MAX(Line),0)+10 FROM C_OpportunityLine WHERE C_SalesOpportunity_ID=?";
			int ii = DB.getSQLValue (get_TrxName(), sql, getC_SalesOpportunity_ID());
			setLine (ii);
		}
...
//			bd = bd.setScale(getPrecision(), BigDecimal.ROUND_HALF_UP);
	...
	public void setHeaderInfo (MSalesOpportunity so)
	{
		m_parent = so;
	}	//	setHeaderInfo
}
