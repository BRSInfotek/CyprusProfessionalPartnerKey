...
import org.cyprusbrs.framework.I_R_InterestArea;
import org.cyprusbrs.framework.MContactInterest;
import org.cyprusbrs.framework.MInterestArea;
...
public class GenerateSubscribeInterestArea extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		if (v_C_Lead_ID == 0)
			throw new CyprusException ("Lead not found -  p_C_Lead_ID=" +  v_C_Lead_ID);
		
		
		if (p_R_InterestArea_ID == null || p_R_InterestArea_ID <= 0) {
		    throw new CyprusException("Interest Area not selected.");
		}
		...
		if(lead.getAD_User_ID()>0)
			AD_UserID=lead.getAD_User_ID();
		else if(lead.getRef_User_ID()>0)
			AD_UserID=lead.getRef_User_ID();
		
		if (AD_UserID == 0) {
		    throw new CyprusException("No valid contact/user linked to the lead.");
		}
		...
		{
			MContactInterest ci = MContactInterest.get(getCtx(), interestArea.getR_InterestArea_ID(), AD_UserID, interestArea.isActive(), get_TrxName());
