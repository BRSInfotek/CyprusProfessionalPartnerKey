...
import org.cyprusbrs.framework.MCity;
...
public class InitialClientSetup extends SvrProcess
{
	...
	private Timestamp p_FiscalYear_Start=null;
	...
	protected void prepare ()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("DateAcct_Start"))		// FiscalYear Start From on initial Client Setup
				p_FiscalYear_Start = para[i].getParameterAsTimestamp();
			else
	...
	protected String doIt () throws Exception
	{
		log.info("InitialClientSetup"
				...
				+ ", CoAFile=" + p_CoAFile
				+ ", FiscalYear_Start=" + p_FiscalYear_Start
		...
		if (   p_ClientName == null || p_ClientName.length() == 0
			...
			|| p_C_Currency_ID <= 0
			|| p_C_Country_ID <= 0
			|| p_CoAFile == null || p_CoAFile.length() == 0 || p_FiscalYear_Start==null
			)
