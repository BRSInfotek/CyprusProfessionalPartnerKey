...
public class RoutingServiceFactory {
  ...
  public static void registerServiceClassname(int AD_Client_ID, String serviceClassname) {
    s_serviceClassnames.put(Integer.valueOf((AD_Client_ID > 0) ? AD_Client_ID : 0), serviceClassname);
  }
  
  private final String getRoutingServiceClassname(int AD_Client_ID) {
    String classname = s_serviceClassnames.get(Integer.valueOf(AD_Client_ID));
    if (classname == null && AD_Client_ID != 0)
      classname = s_serviceClassnames.get(Integer.valueOf(0)); 
    if (classname == null)
      classname = "org.eevolution.model.impl.DefaultRoutingServiceImpl"; 
    return classname;
  }
  
  public RoutingService getRoutingService(int AD_Client_ID) {
    RoutingService service = s_services.get(Integer.valueOf(AD_Client_ID));
    if (service != null)
      return service; 
    String classname = getRoutingServiceClassname(AD_Client_ID);
    try {
      Class<? extends RoutingService> cl = (Class)getClass().getClassLoader().loadClass(classname);
      service = cl.newInstance();
      s_services.put(Integer.valueOf(AD_Client_ID), service);
    } catch (Exception e) {
