...
{	
	...
	/** Period						*/
	private int					m_M_Product_ID = -1;
	
	private BigDecimal m_trxCost=Env.ZERO;	// Updated by Anshul 20240730
	...
	{
		/// code commented by Mukesh @20250630
...
//		}
//		return 0;
		/// code updated by Mukesh @20250630
		if (m_M_Product_ID == -1)
		{
			int index = p_po.get_ColumnIndex("M_Product_ID");
			if (index != -1)
	...
	{
		int index = p_po.get_ColumnIndex("M_InOutLine_ID");
		if (index != -1)
		{
			Integer ii = (Integer)p_po.get_Value(index);
			if (ii != null)
	...
	{
		int index = this.p_po.get_ColumnIndex("BasisType");
		if(index != -1)
	...
	{
		int index = this.p_po.get_ColumnIndex("MFG_WorkOrderOperation_ID");
		if(index != -1)
		{
	...
	// updated by Anshul @30072024 to set TrxCost 
		...
		// updated by Anshul @30072024 to get TrxCost 
		/**
		 *  Transaction Cost
		 *  @return transaction Qty
		 */
