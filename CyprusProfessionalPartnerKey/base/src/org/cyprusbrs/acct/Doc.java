...
import org.cyprusbrs.framework.MConversionRate;
import org.cyprusbrs.framework.MDocType;
import org.cyprusbrs.framework.MNote;
...
public abstract class Doc
{
	...
	/** Work Order    */
	public static final String	DOCTYPE_MFGWorkOrder	= "WOO";
	/** Job Work Receipt    */
	public static final String	DOCTYPE_JobWorkReceipt	= "JWR";  /// Added by Mukesh	
	/** Job Work Issued    */
	public static final String	DOCTYPE_JobWorkIssued	= "JWI";  /// Added by Mukesh
	
	public static final String	DOCTYPE_ManufacturingCostCollector	= "MCC";  /// Added by Mukesh @20250711
	 ...
	 * 	@param trxName trx
	 */
	protected Doc (MAcctSchema[] ass, Class<?> clazz, ResultSet rs, String defaultDocumentType, String trxName)
	{
	 ...
	 * @throws Exception 
	 */
	private final String postLogic (int index) throws Exception
	{
		log.info("(" + index + ") " + p_po);
	...
	public static final int 	AMTTYPE_Withholding  = 3;

	/** Quantity								*/
	private BigDecimal			m_qty = null;
	
	public static final int 	AMTTYPE_TCS  = 4;
	 
	/** Source Amounts (may not all be used)	*/

	private BigDecimal[]		m_Amounts = new BigDecimal[5];
	...
	/**
	 * Cyprus Manufacturing Accounts by Mukesh @20220908
	 */
	/** Manufacturing Accounts - Work Order Material */
	public static final int     ACCTTYPE_WorkOrderMaterial = 17730;
	/** Manufacturing Accounts - Work Order Resource */
	public static final int     ACCTTYPE_WorkOrderResource = 17750;
	/** Manufacturing Accounts - Work Order Material Overhead */
	public static final int     ACCTTYPE_WorkOrderMaterialOverhead = 17740;
	/** Manufacturing Accounts - Work Order Material Overhead */
	public static final int     ACCTTYPE_ProductionResourceAbsorption = 79800;

	/** Manufacturing Accounts - Work Center Overhead */
	public static final int     ACCTTYPE_WorkCenterOverhead = 17760;
	/** Manufacturing Accounts - Work Order Material Variance */
	public static final int     ACCTTYPE_WorkOrderMaterialVariance = 53010;
	/** Manufacturing Accounts - Work Order Resource Variance */
	public static final int     ACCTTYPE_WorkOrderResrouceVariance = 53030;
	/** Manufacturing Accounts - Work Order Overhead Variance */
	public static final int     ACCTTYPE_WorkOrderOverheadVariance = 53030;
	
	/** Job Work Accounts - Job Order Issued... This code will be removed */
	public static final int     ACCTTYPE_JobWorkProgress = 53032;
	
	/** Job Work Accounts - Job Order Issued */
	public static final int     ACCTTYPE_OutsideProcessing = 51130;
	
	/** Withholding Accounts - Withholding */
	public static final int     ACCTTYPE_Withholding = 21700;
	
	/** Product Asset Accounts - Product Asset */
	public static final int     ACCTTYPE_P_Asset_Acct = 14120;
	
	/** Product Asset Accounts - Product Asset */
	public static final int     ACCTTYPE_WorkInProcess = 14130;
	
	/** TCS Accounts - Tax Collected At Source */
	public static final int     ACCTTYPE_TCS = 25100;
...
/**
	...
	public int getValidCombination_ID (int AcctType, MAcctSchema as)
	{
		...
		{
			System.out.println(getAmount(AMTTYPE_Charge));
		...
		else if (AcctType == ACCTTYPE_JobWorkProgress)
		{
			sql = "SELECT J_JobWorkProgress_Acct FROM C_BP_Vendor_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID=?";
			para_1 = getC_BPartner_ID();
		}
		/// Created by Mukesh @20250624
		else if (AcctType == ACCTTYPE_OutsideProcessing)
		{
			sql = "SELECT P_OutsideProcessing_Acct FROM C_BP_Vendor_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID=?";
			para_1 = getC_BPartner_ID();
		}
		...
		/**
		 * Manufacturing code by Mukesh
		 */
		else if (AcctType == ACCTTYPE_WorkOrderMaterial)
		{
			sql = "SELECT WO_Material_Acct FROM MFG_WorkOrderClass_Acct WHERE C_AcctSchema_ID=?";
			para_1 = -1;
		}
		else if (AcctType == ACCTTYPE_WorkOrderResource)
		{
			sql = "SELECT WO_Resource_Acct FROM MFG_WorkOrderClass_Acct WHERE C_AcctSchema_ID=?";
			para_1 = -1;
		}
		else if (AcctType == ACCTTYPE_WorkOrderMaterialOverhead)
		{
			sql = "SELECT WO_MaterialOverhd_Acct FROM MFG_WorkOrderClass_Acct WHERE C_AcctSchema_ID=?";
			para_1 = -1;
		}
		/* Withholding code added by Anshul
		 */
		else if (AcctType == ACCTTYPE_Withholding)
		{
			sql = "SELECT Withholding_Acct FROM C_Withholding_Acct WHERE C_AcctSchema_ID=?";
			para_1 = -1;
		}
		
		/* Product Assed code added by Mukesh @20250630
		 */
		else if (AcctType == ACCTTYPE_P_Asset_Acct)
		{
			sql = "SELECT P_Asset_Acct FROM M_Product_Acct WHERE C_AcctSchema_ID=?";
			para_1 = -1;
		}
		/* Work in process code added by Mukesh @20250711
		 */
		else if (AcctType == ACCTTYPE_WorkInProcess)
		{
			sql = "SELECT P_WIP_Acct FROM M_Product_Acct WHERE C_AcctSchema_ID=?";
			para_1 = -1;
		}
		else if (AcctType == ACCTTYPE_TCS)
		{
			sql = "SELECT TCS_Acct FROM M_TCS_Acct WHERE C_AcctSchema_ID=?";
			para_1 = -1;
		}
		
//		else if (AcctType == ACCTTYPE_JobWorkProgress)
//		{
//			sql = "SELECT J_JobWorkProgress_Acct FROM C_BP_Vendor_Acct WHERE C_AcctSchema_ID=?";
//			para_1 = -1;
//		}
		/*
		 * else if (AcctType == ACCTTYPE_ProductionResourceAbsorption) { sql =
		 * "SELECT P_Resource_Absorption_Acct FROM M_Product_Acct WHERE C_AcctSchema_ID=?"
		 * ; para_1 = -1; }
		 * 
		 * /// Regarding Work Order Posting else if (AcctType ==
		 * ACCTTYPE_WorkCenterOverhead) { sql =
		 * "SELECT WC_Overhead_Acct FROM MFG_WorkCenter_Acct WHERE C_AcctSchema_ID=?";
		 * para_1 = -1; } else if (AcctType == ACCTTYPE_WorkOrderMaterialVariance) { sql
		 * =
		 * "SELECT WO_MaterialVariance_Acct FROM MFG_WorkOrderClass_Acct  WHERE C_AcctSchema_ID=?"
		 * ; para_1 = -1; } else if (AcctType == ACCTTYPE_WorkOrderResrouceVariance) {
		 * sql =
		 * "SELECT WO_ResourceVariance_Acct FROM MFG_WorkOrderClass_Acct  WHERE C_AcctSchema_ID=?"
		 * ; para_1 = -1; } else if (AcctType == ACCTTYPE_WorkOrderOverheadVariance) {
		 * sql =
		 * "SELECT WO_OverhdVariance_Acct FROM MFG_WorkOrderClass_Acct  WHERE C_AcctSchema_ID=?"
		 * ; para_1 = -1; }
		 */
		
		// End of code of Manufacturing
		...
		try
		{
			//System.out.println(as.getC_AcctSchema_ID()+"  "+para_1+" Sql Query : "+sql);
			pstmt = DB.prepareStatement(sql, null);
	 ...
	 * @throws Exception 
	 */
	public abstract ArrayList<Fact> createFacts (MAcctSchema as) throws Exception;
