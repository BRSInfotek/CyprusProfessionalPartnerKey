...
public class Doc_MFGWorkOrderTransaction extends Doc {
  ...
  public Doc_MFGWorkOrderTransaction(MAcctSchema[] ass, ResultSet rs, String trx) {
  ...
  private MAccount[] getAccounts(MCostElement ce, MAcctSchema as, DocLine line, boolean isScrap) {
    ...
    if (this.DocTxnType.equals(X_MFG_WorkOrderTransaction.WORKORDERTXNTYPE_ComponentIssueToWorkOrder) || this.DocTxnType.equals(X_MFG_WorkOrderTransaction.WORKORDERTXNTYPE_ResourceUsage)) {
    	if (ce.getCostElementType().equals(X_M_CostElement.COSTELEMENTTYPE_Material)) {
    		...
    		// drAcct = getAccount(1773, as); // Compiere code by Mukesh @20220909
    		crAcct = line.getAccount(ProductCost.ACCTTYPE_P_Asset, as);
    		drAcct = getAccount(ACCTTYPE_WorkOrderMaterial, as); //
    		...
    		// drAcct = getAccount(1774, as); // Compiere code by Mukesh @20220909
    		crAcct = line.getAccount(ProductCost.ACCTTYPE_P_MethodChangeVariance, as);	  
    		drAcct = getAccount(ACCTTYPE_WorkOrderMaterialOverhead, as);
    		...
    		//  drAcct = getAccount(1775, as);// Compiere code Commented by Mukesh @20220909
    		crAcct = line.getAccount(ProductCost.ACCTTYPE_P_WorkInProcess, as);
    		drAcct = getAccount(ACCTTYPE_WorkOrderResource, as); // Need to update correct DrAcct after discussion with Surya
    		this.resourceIn = BigDecimal.ONE;
    	}
      
   // New added in Cyprus Not exist in Compiere (Not Used in Cyprus)
      else if (ce.getCostElementType().equals(X_M_CostElement.COSTELEMENTTYPE_OutsideProcessing)) {
          crAcct = line.getAccount(5154, as);  // Need to update correct CrAcct after discussion with Surya
          drAcct = getAccount(5254, as); // Need to update correct DrAcct after discussion with Surya
          this.resourceIn = BigDecimal.ONE;	
      }
//    	(Not Used in Cyprus)
      ...
      if (ce.getCostElementType().equals(X_M_CostElement.COSTELEMENTTYPE_Material)) {
        drAcct = line.getAccount(ProductCost.ACCTTYPE_P_Asset, as);
        crAcct = getAccount(ACCTTYPE_WorkOrderMaterial, as);
        this.materialOut = BigDecimal.ONE;
      } else if (ce.getCostElementType().equals(X_M_CostElement.COSTELEMENTTYPE_BurdenMOverhead)) {
        drAcct = line.getAccount(ProductCost.ACCTTYPE_P_MethodChangeVariance, as);
        crAcct = getAccount(ACCTTYPE_WorkOrderMaterialOverhead, as);
        this.materialOverhdOut = BigDecimal.ONE;
      } else if (ce.getCostElementType().equals(X_M_CostElement.COSTELEMENTTYPE_Resource)) {
        drAcct = line.getAccount(ProductCost.ACCTTYPE_P_WorkInProcess, as);
        crAcct = getAccount(ACCTTYPE_WorkOrderResource, as);
