...
import org.cyprusbrs.framework.MCurrency;
import org.cyprusbrs.framework.MInvoice;
import org.cyprusbrs.framework.MInvoiceLine;
import org.cyprusbrs.framework.MLandedCostAllocation;
...
public class Doc_Invoice extends Doc
{
	...
	protected String loadDocumentDetails ()
	{
		...
		// updated by Anshul @22-04-2025 for withholding tax amt
		setAmount(Doc.AMTTYPE_Withholding, invoice.get_ValueAsBigDecimal("TotalWithholdingAmt"));
		BigDecimal TCSTotalAmt = DB.getSQLValueBD(null, "SELECT TCSTotalAmt FROM C_Invoice WHERE C_Invoice_ID=?", invoice.getC_Invoice_ID());
		setAmount(Doc.AMTTYPE_TCS, TCSTotalAmt);
		
		m_Reversal_ID = invoice.getReversal_ID();//store original (voided/reversed) document
		m_DocStatus = invoice.getDocStatus();
		//	Contained Objects
		m_taxes = loadTaxes();
	...
	private DocLine[] loadLines (MInvoice invoice)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			try {
			docLine.setReversalLine_ID((Integer)line.get_Value("ReversalLine_ID"));
			}catch(Exception e) {
				System.out.println(e.getMessage());
				log.warning("Doc Invoice Reverse is not saved");}
			//	Qty
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
			...
			|| getDocumentType().equals(DOCTYPE_ARProForma))
		{
			...
			{
				if (m_DocStatus.equals(MInvoice.DOCSTATUS_Reversed) 
						&& m_Reversal_ID !=0 && amt.compareTo(Env.ZERO) < 0)
				{					
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				{					
					if (!cr.updateReverseLine (MInvoice.Table_ID, 
							m_Reversal_ID, line.getReversalLine_ID(),Env.ONE))
					{
						p_Error = "Original Invoice not posted yet";
						return null;
					}
				}
			}
			...
			// Withholding
			int withholding_ID = getValidCombination_ID (Doc.ACCTTYPE_Withholding, as);
			BigDecimal withholdingAmt = getAmount(Doc.AMTTYPE_Withholding);
			if (withholdingAmt != null && withholdingAmt.signum() != 0)
			{
				if (m_DocStatus.equals(MInvoice.DOCSTATUS_Reversed) 
						&& m_Reversal_ID !=0 && withholdingAmt.compareTo(Env.ZERO) < 0)
				{					
					fact.createLine(null, MAccount.get(getCtx(), withholding_ID),
							getC_Currency_ID(), null, withholdingAmt.negate());
				}
				else
				{
					fact.createLine(null, MAccount.get(getCtx(), withholding_ID),
							getC_Currency_ID(), withholdingAmt, null);
				}
				
			}
			
			// Tax Collected At Source
			int TCS_ID = getValidCombination_ID (Doc.ACCTTYPE_TCS, as);
			BigDecimal TCSAmt = getAmount(Doc.AMTTYPE_TCS);
			if (TCSAmt != null && TCSAmt.signum() != 0)
			{
				if (m_DocStatus.equals(MInvoice.DOCSTATUS_Reversed) 
						&& m_Reversal_ID !=0 && TCSAmt.compareTo(Env.ZERO) < 0)
				{					
					fact.createLine(null, MAccount.get(getCtx(), TCS_ID),
							getC_Currency_ID(), TCSAmt.negate(), null);
				}
				else
				{
					fact.createLine(null, MAccount.get(getCtx(), TCS_ID),
							getC_Currency_ID(), null, TCSAmt);
				}
			...
			if (grossAmt!=null && grossAmt.signum() != 0)
			{
				if (m_DocStatus.equals(MInvoice.DOCSTATUS_Reversed) 
						&& m_Reversal_ID !=0 && grossAmt.compareTo(Env.ZERO) < 0)
				{					
					fact.createLine(null, MAccount.get(getCtx(), receivables_ID),
							getC_Currency_ID(), null, grossAmt.negate());
				}
				else
				{
					fact.createLine(null, MAccount.get(getCtx(), receivables_ID),
			...
			if (serviceAmt!=null && serviceAmt.signum() != 0)
			{
				if (m_DocStatus.equals(MInvoice.DOCSTATUS_Reversed) 
						&& m_Reversal_ID !=0 && serviceAmt.compareTo(Env.ZERO) < 0)
				{					
					fact.createLine(null, MAccount.get(getCtx(), receivablesServices_ID),
							getC_Currency_ID(), null, serviceAmt.negate());
				}
				else
				{
					fact.createLine(null, MAccount.get(getCtx(), receivablesServices_ID),
		...
		else if (getDocumentType().equals(DOCTYPE_ARCredit))
		{
			...
			if (amt != null && amt.signum() != 0)
			{
				...
				{					
					fact.createLine(null, getAccount(Doc.ACCTTYPE_Charge, as),
							getC_Currency_ID(), null, amt.negate());
				}
				else
				{
					fact.createLine(null, getAccount(Doc.ACCTTYPE_Charge, as),
			...
			if (withholdingAmt!=null && withholdingAmt.signum() != 0)
			{
				...
				{					
					fact.createLine(null, MAccount.get(getCtx(), withholding_ID),
							getC_Currency_ID(), withholdingAmt.negate(), null);
				}
				else
				{
					fact.createLine(null, MAccount.get(getCtx(), withholding_ID),
							getC_Currency_ID(), null, withholdingAmt);
				}
			}
			...
			if (TCSAmt!=null && TCSAmt.signum() != 0)
			{
				...
				{					
					fact.createLine(null, MAccount.get(getCtx(), TCS_ID),
							getC_Currency_ID(), null, TCSAmt.negate());
				}
				else
				{
					fact.createLine(null, MAccount.get(getCtx(), TCS_ID),
							getC_Currency_ID(), TCSAmt, null);
				}
			}
		...
		else if (getDocumentType().equals(DOCTYPE_APInvoice))
		{
			...
			//  Charge          DR
			if(getAmount(Doc.AMTTYPE_Charge)!=null)
			{
				if (m_DocStatus.equals(MInvoice.DOCSTATUS_Reversed) 
						&& m_Reversal_ID !=0 && getAmount(Doc.AMTTYPE_Charge).compareTo(Env.ZERO) < 0)
				{
					 fact.createLine(null, getAccount(Doc.ACCTTYPE_Charge, as),
								getC_Currency_ID(), null, getAmount(Doc.AMTTYPE_Charge).negate());				
				}
				else
				{
					fact.createLine(null, getAccount(Doc.ACCTTYPE_Charge, as),
			...
			for (int i = 0; i < m_taxes.length; i++)
			{
				BigDecimal taxAmt = m_taxes[i].getAmount();
				if(taxAmt == null)
				{
					taxAmt = Env.ZERO;
				}
				...
				FactLine tl = null;
				if (m_DocStatus.equals(MInvoice.DOCSTATUS_Reversed) 
						&& m_Reversal_ID !=0 && taxAmt.compareTo(Env.ZERO) < 0)
				{
					tl = fact.createLine(null, m_taxes[i].getAccount(m_taxes[i].getAPTaxType(), as),
							getC_Currency_ID(),null , m_taxes[i].getAmount().negate());					
				}
				else
				{
					 tl = fact.createLine(null, m_taxes[i].getAccount(m_taxes[i].getAPTaxType(), as),
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				{
					dr = fact.createLine (line, line.getAccount(ProductCost.ACCTTYPE_P_Expense, as),
				...
				if (!landedCost)
				{
					...
					if (as.isTradeDiscountPosted() && !line.isItem())
					{
						...
						if (discount != null && discount.signum() != 0)
						{
							...
							dr = fact.createLine (line, tradeDiscountReceived,
									getC_Currency_ID(), null, dAmt);
						}
					}
					dr = fact.createLine (line, expense,
						getC_Currency_ID(), amt, null);
					if (!line.isItem())
					{
						grossAmt = grossAmt.subtract(amt);
					...
					// Code commneted by Anshul @20240729
...
//							line.getDescription(), getTrxName());
						...
						&& m_Reversal_ID !=0 && line.getReversalLine_ID() != 0)
				{
					...
					if (!dr.updateReverseLine (MInvoice.Table_ID, 
							m_Reversal_ID, line.getReversalLine_ID(),Env.ONE))
					{
						p_Error = "Original Invoice not posted yet";
						return null;
		...
		else if (getDocumentType().equals(DOCTYPE_APCredit))
		{
			...
			if(getAmount(Doc.AMTTYPE_Charge)!=null)
			{
				...
				{					
					fact.createLine (null, getAccount(Doc.ACCTTYPE_Charge, as),
							getC_Currency_ID(), getAmount(Doc.AMTTYPE_Charge).negate(), null);
				}
				else
				{
					fact.createLine (null, getAccount(Doc.ACCTTYPE_Charge, as),
			...
			for (int i = 0; i < m_taxes.length; i++)
			{
				if(m_taxes[i].getAmount()!=null)
				{
					FactLine tl= null;
					if (m_DocStatus.equals(MInvoice.DOCSTATUS_Reversed) 
							&& m_Reversal_ID !=0 && m_taxes[i].getAmount().compareTo(Env.ZERO) < 0)
					{					
						 tl = fact.createLine (null, m_taxes[i].getAccount(m_taxes[i].getAPTaxType(), as),
									getC_Currency_ID(), m_taxes[i].getAmount().negate(), null);
					}
					else
					{
						 tl = fact.createLine (null, m_taxes[i].getAccount(m_taxes[i].getAPTaxType(), as),
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				if (landedCost && as.isExplicitCostAdjustment())
				{
					cr = fact.createLine (line, line.getAccount(ProductCost.ACCTTYPE_P_Expense, as),
						getC_Currency_ID(), null, line.getAmtSource());
					//
					FactLine fl = fact.createLine (line, line.getAccount(ProductCost.ACCTTYPE_P_Expense, as),
						getC_Currency_ID(), line.getAmtSource(), null);
				...
				if (!landedCost)
				{
					...
					if (as.isTradeDiscountPosted() && !line.isItem())
					{
						...
						if (discount != null && discount.signum() != 0)
						{
							...
							dAmt = discount;
							MAccount tradeDiscountReceived = line.getAccount(ProductCost.ACCTTYPE_P_TDiscountRec, as);
							cr = fact.createLine (line, tradeDiscountReceived,
									getC_Currency_ID(), dAmt, null);
						}
					}
					cr = fact.createLine (line, expense,
						getC_Currency_ID(), null, amt);
					if (!line.isItem())
					{
						grossAmt = grossAmt.subtract(amt);
...
//							line.getDescription(), getTrxName());
	...
	private boolean landedCost (MAcctSchema as, Fact fact, DocLine line, boolean dr) 
	{
		...
		return true;
		// Commented by Anshul @20240722 Shift all functionality in Complete button
		/** 
