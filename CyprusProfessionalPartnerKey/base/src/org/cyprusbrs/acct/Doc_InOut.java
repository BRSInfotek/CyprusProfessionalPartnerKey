...
public class Doc_InOut extends Doc
{
	...
	private DocLine[] loadLines(MInOut inout)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			System.out.println(" getDocumentType() "+getDocumentType());
			if(!getDocumentType().equals(DOCTYPE_JobWorkReceipt))	// Added by Mukesh
			...
			// updated by Anshul @30072024 to set TrxCost 
			BigDecimal trxCost = line.getTrxCost();
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		if (getDocumentType().equals(DOCTYPE_MatShipment) && isSOTrx())
		{
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				// Commented by ANshul to shift in Complete button
				/**
		...
		else if ( getDocumentType().equals(DOCTYPE_MatReceipt) && isSOTrx() )
		{
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				// Commented by Anshul @20240708- replacing the logic on Complete button
//				if (line.getM_Product_ID() != 0)
//				{
//					MCostDetail.createShipment(as, line.getAD_Org_ID(), 
//						line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(),
		...
		else if (getDocumentType().equals(DOCTYPE_MatReceipt) && !isSOTrx())
		{
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				if (MAcctSchema.COSTINGMETHOD_AveragePO.equals(costingMethod) ||
					MAcctSchema.COSTINGMETHOD_AverageInvoice.equals(costingMethod) || // Added by Anshul @13072024
					MAcctSchema.COSTINGMETHOD_LastInvoice.equals(costingMethod) ||    // Added by Anshul @13072024	
					MAcctSchema.COSTINGMETHOD_LastPOPrice.equals(costingMethod) || 
					MAcctSchema.COSTINGMETHOD_Fifo.equals(costingMethod) ||	// Added by Anshul @13072024
					MAcctSchema.COSTINGMETHOD_Lifo.equals(costingMethod) // Added by Anshul @13072024
					)
				{
					int C_OrderLine_ID = line.getC_OrderLine_ID();
					// Low - check if c_orderline_id is valid
					if ((C_OrderLine_ID > 0 && !as.isPostPOVariance()) || (C_OrderLine_ID > 0 && (MAcctSchema.COSTINGMETHOD_Fifo.equals(costingMethod) ||
							MAcctSchema.COSTINGMETHOD_Lifo.equals(costingMethod)))) // Added && !as.isPostPOVariance() by Anshul 20240713
					{
...
//				}
		 ...
		 //	  *** Job work Receipt - By Mukesh @20250416
		else if (getDocumentType().equals(DOCTYPE_JobWorkReceipt))
		{
			int C_Currency_ID = as.getC_Currency_ID();

			MInOut inout = (MInOut)getPO();
			DocLine[] mlines = loadChargeLines(inout);
			BigDecimal chargeAmt=Env.ZERO;
			System.out.println("Length : "+mlines.length);
			for(DocLine line:mlines)
			{		
				System.out.println("Charge Amt "+line.getTrxCost());
				chargeAmt=chargeAmt.add(line.getTrxCost());
					
//					cr=fact.createLine(null, getAccount(Doc.ACCTTYPE_Charge, as),
//							getC_Currency_ID(), null, line.getTrxCost());
//					System.out.println("Charge Details : "+cr);

					///CR				
//					cr = fact.createLine(line,
//							getAccount(Doc.ACCTTYPE_Charge, as),
//							C_Currency_ID, null , line.getTrxCost());
					//
//					if (cr == null)
//					{
//						p_Error = "DR not created: " + line;
...
//					cr.setLocationFromBPartner(getC_BPartner_Location_ID(), true);   //  from Loc
//					cr.setLocationFromLocator(line.getM_Locator_ID(), false);   //  to Loc
//					cr.setQty(line.getQty().abs());
				
			}
			setAmount(Doc.AMTTYPE_Charge, chargeAmt);
			System.out.println("Length of Lines "+p_lines.length);

			log.log(Level.INFO, "Length of Lines "+p_lines.length);
			for (int i = 0; i < p_lines.length; i++)
			{
				//
				DocLine line = p_lines[i];
				//log.log(Level.INFO, "Doc Lines : "+line.toString());
				MInOutLine mLine=new MInOutLine(getCtx(), line.getM_InOutLine_ID(), getTrxName());
				
			//	MProduct product = line.getProduct();
				//log.log(Level.INFO, "Doc Lines : "+line.toString());
				//System.out.println(costs+" Cost of Product "+product.getName());
				
			//  DR
				if(line.getM_Product_ID()>0)
				{
					BigDecimal costs = mLine.getTrxCost().multiply(mLine.getQtyEntered());
					costs=costs.add(chargeAmt);
					dr = fact.createLine(line,
							line.getAccount(ProductCost.ACCTTYPE_P_Asset, as),
							C_Currency_ID, costs, null);
					if (dr == null)
					...
					BigDecimal costsJ = mLine.getTrxCost().multiply(mLine.getQtyEntered());
					if(costsJ==null)
						costsJ=Env.ZERO;
					///CR				
					cr = fact.createLine(line,
							getAccount(Doc.ACCTTYPE_OutsideProcessing, as),
							C_Currency_ID, null , costsJ);
					//
					if (cr == null)
					{
						p_Error = "DR not created: " + line;
				...
				else // For Charge Entry
				{
					BigDecimal costs = mLine.get_ValueAsBigDecimal("Price").multiply(mLine.getQtyEntered());
					///CR
					line.setAmount(costs.negate());
					Integer chargeID = (Integer) mLine.get_Value("C_Charge_ID");
					setC_Charge_ID(chargeID);
				//  Header Charge           CR
					setAmount(Doc.AMTTYPE_Charge, costs.negate());
...
//					if (cr == null)
//					{
//						p_Error = "CR not created Charge: " + line;
//						log.log(Level.WARNING, p_Error);
//						return null;
//					}
//					cr.setM_Locator_ID(line.getM_Locator_ID());
...
//					cr.setQty(line.getQty().negate());
		 ...
		 //	  *** Job work Issued - By Mukesh @20250416
		else if (getDocumentType().equals(DOCTYPE_JobWorkIssued))
		{
			log.log(Level.INFO, "Length of Lines "+p_lines.length);
	 ...
	 *  @return DocLine Array
	 */
	private DocLine[] loadChargeLines(MInOut inout)
	{
		ArrayList<DocLine> list = new ArrayList<DocLine>();
		MInOutLine[] lines = inout.getLines(false);
		for (int i = 0; i < lines.length; i++)
		{
			...
			BigDecimal Qty = line.getMovementQty();
//			docLine.setReversalLine_ID(line.getReversalLine_ID());
			docLine.setQty (Qty, true);    //  sets Trx and Storage Qty
			// updated by Anshul @30072024 to set TrxCost 
			BigDecimal trxCost = line.get_ValueAsBigDecimal("Price");
				docLine.setTrxCost(trxCost.multiply(Qty));
				docLine.setAmount(trxCost.multiply(Qty));
