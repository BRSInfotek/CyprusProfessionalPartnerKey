...
import org.cyprusbrs.framework.MBankAccount;
...
public class Doc_Payment extends Doc
{
	...
	private BigDecimal amountBeforeWithholding = Env.ZERO; 
	...
	protected String loadDocumentDetails ()
	{
		...
		setAmount(Doc.AMTTYPE_Withholding, pay.get_ValueAsBigDecimal("WithholdingTaxAmt"));
		amountBeforeWithholding = pay.get_ValueAsBigDecimal("PayAmtbeforeWithholding");
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		if (getDocumentType().equals(DOCTYPE_ARReceipt))
		{
			...
			if (getC_Charge_ID() != 0)
			{
				...
				BigDecimal withholdingTaxAmt =  getAmount(Doc.AMTTYPE_Withholding);
				
				if(withholdingTaxAmt.signum() != 0)
				{
					fl = fact.createLine(null,  MAccount.get(getCtx(), withholding_ID),
							getC_Currency_ID(),withholdingTaxAmt, null);
				}
		...
		else if (getDocumentType().equals(DOCTYPE_APPayment))
		{
			...
			{
				fl = fact.createLine(null,  MAccount.get(getCtx(), withholding_ID),
						getC_Currency_ID(),null, withholdingTaxAmt);
			}
