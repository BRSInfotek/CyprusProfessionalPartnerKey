...
import org.cyprusbrs.framework.X_I_ElementValue;
...
public class ImportAccount extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		try
		{
			 pstmt1 = DB.prepareStatement(sql.toString(), get_TrxName());
			...
			while (rs1.next())
			{
				updateStmt.setInt(1, rs1.getInt(1));		//	Parent
				updateStmt.setInt(2, rs1.getInt(2));		//	SeqNo (assume sequenec in import is the same)
				updateStmt.setInt(3, rs1.getInt(3));		//	Tree
				updateStmt.setInt(4, rs1.getInt(4));		//	Node
				try
				{
				...
				if (no == 0)
					log.info("Parent not found for " + rs1.getString(5));
			}
			rs1.close();
			pstmt1.close();
		}
