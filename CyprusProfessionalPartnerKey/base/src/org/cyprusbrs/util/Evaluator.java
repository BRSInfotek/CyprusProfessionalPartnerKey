...
//	}	//	evaluateLogicTuple
	...
	private static boolean evaluateLogicTuple(Evaluatee source, String logic) {
		// First, check for multi-character operators
	    String[] operators = {"!=", "<>", ">=", "<=", "=", ">", "<", "^","!"};
	    String foundOperator = null;
	    int operatorPos = -1;
	    
	    // Find the first occurring operator (longest first to catch multi-character operators)
	    for (String op : operators) {
	        int pos = logic.indexOf(op);
	        if (pos > 0) {
	            foundOperator = op;
	            operatorPos = pos;
	            break;
	    ...
	    if (foundOperator == null) {
	        s_log.log(Level.SEVERE, "No valid operator found in logic tuple: " + logic);
	        return false;
	    }
	    
	    // Split the string around the operator
	    String first = logic.substring(0, operatorPos).trim();
	    String second = logic.substring(operatorPos + foundOperator.length()).trim();
	    
	    // First Part
	    String firstEval = first.trim();
	    if (first.indexOf('@') != -1) {      // variable
	        first = first.replace('@', ' ').trim();           // strip 'tag'
	        firstEval = source.get_ValueAsString(first);      // replace with its value
	    }
	    firstEval = firstEval.replace('\'', ' ').replace('"', ' ').trim();   // strip ' and "
	    ...
	    if (second.indexOf('@') != -1) {     // variable
	        second = second.replace('@', ' ').trim();          // strip tag
	        secondEval = source.get_ValueAsString(second);     // replace with its value
	    }
	    secondEval = secondEval.replace('\'', ' ').replace('"', ' ').trim(); // strip ' and "
	    ...
	    // Logical Comparison
	    boolean result = evaluateLogicTuple(firstEval, foundOperator, secondEval);
	    
	    if (CLogMgt.isLevelFinest())
	        s_log.finest(logic 
	            + " => \"" + firstEval + "\" " + foundOperator + " \"" + secondEval + "\" => " + result);
