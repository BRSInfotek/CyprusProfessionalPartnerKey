...
import org.cyprusbrs.db.CConstraint;
...
import org.cyprusbrs.framework.MSequence;
import org.cyprusbrs.framework.MSysConfig;
...
//    Check Build
	...
	public static void createT_Selection_Browse(int AD_PInstance_ID, LinkedHashMap<Integer, 
	  ...
	  private static Object[][] executeQuery(PreparedStatement pstmt) throws SQLException {
		    ...
		    return result.<Object[]>toArray(new Object[0][]);
		  }
	  ...
	  private static class UpdateStats {
		    ...
		    Integer numExecutions = Integer.valueOf(0);
		    
		    Integer numRecords = Integer.valueOf(0);
		    
		    Long timeSpent = Long.valueOf(0L);
		  }
	  ...
	  public static int executeUpdate(Trx trx, String sql, List<Object> params) throws Exception {
		    return executeUpdate(trx, sql, params.toArray());
		  }
	  
	  public static int executeUpdate(Trx trx, String sql, Object... params) throws Exception {
		    List<Object[]> bulkParams = new ArrayList();
		    bulkParams.add(params);
		    return executeBulkUpdate(trx, sql, bulkParams, false, false);
		  }
	  
	  public static int executeBulkUpdate(Trx trx, String sql, List<Object[]> bulkParams, boolean ignoreError, boolean bulkSQL) throws Exception {
		    long time = System.currentTimeMillis();
		    if (sql == null || sql.length() == 0)
		      throw new IllegalArgumentException("Required parameter missing - " + sql); 
		//    CPreparedStatement pstmt = new CPreparedStatement(1003, 1008, sql, trx.getTrxName());
...
//		        for (int j = (count - 1) / JDBCBatchSize * JDBCBatchSize; j < ((count - 1) / JDBCBatchSize + 1) * JDBCBatchSize && j < bulkParams.size(); j++) {
		        for (int j = (count - 1)  ; j < bulkParams.size(); j++) {
		      ...
		      if (isMSSQLServer() && sql.startsWith("ALTER TABLE ") && e.getMessage().indexOf("may cause cycles or multiple") > 0) {
		    ...
		    if (s_isLoggingUpdates) {
		      ...
		      UpdateStats updateStats1 = stats;
		      updateStats1.numExecutions = Integer.valueOf(updateStats1.numExecutions.intValue() + 1);
		      updateStats1 = stats;
		      updateStats1.numRecords = Integer.valueOf(updateStats1.numRecords.intValue() + bulkParams.size());
		      updateStats1 = stats;
		      updateStats1.timeSpent = Long.valueOf(updateStats1.timeSpent.longValue() + time);
		    } 
		    return total;
		  }
	  ...
	  private static int executeTrigger(String sql, Trx trx) throws Exception {
		    int no = -1;
		    String newSQL = null;
		    ...
		    if (st.length > 3) {
		      ...
		      no = executeUpdate(trx, preSQL, new Object[0]);
		      if (no > -1)
		        log.log(Level.INFO, "Successfully executed pre-create SQL: " + preSQL); 
		    } 
		    no = executeUpdate(trx, newSQL, new Object[0]);
		    if (no > -1) {
		      ...
		      if (iret > -1) {
		        ...
		        log.warning(("      but unable to log for " + newSQL + localTrx == null) ? " - Connection is null." : (" - " + localTrx.toString()));
		      } 
		      localTrx.close();
		    } 
		    return no;
	  ...
	  public static Object[][] executeQuery(Trx trx, String SQL, Object... params) throws Exception {
	  ...
	  public static void startLoggingUpdates() {
		    s_isLoggingUpdates = true;
		  }
		  
		  public static String stopLoggingUpdates(int numRecordsThreshold) {
		    s_isLoggingUpdates = false;
		    StringBuilder s = new StringBuilder();
		    List<UpdateStats> stats = new ArrayList<UpdateStats>(s_updateStats.values());
		    Collections.sort(stats, new Comparator<UpdateStats>() {
		          public int compare(DB.UpdateStats o1, DB.UpdateStats o2) {
		    ...
		    for (UpdateStats stat : stats) {
		      if (stat.numRecords.intValue() > numRecordsThreshold) {
		        f.format("%8d", new Object[] { stat.timeSpent });
		        s.append(" ms spent in ");
		        f.format("%6d", new Object[] { stat.numExecutions });
		        s.append(" updates of ");
		        f.format("%8.1f", new Object[] { Float.valueOf(stat.numRecords.intValue() / stat.numExecutions.intValue()) });
