...
public final class EMail implements Serializable
{
	...
	private MimeMessage m_message = null;
	/** Context - may be null		*/
	...
	public String send()
	{
		...
		if ("S".equalsIgnoreCase((String)client.get_Value("OAuthProvider"))) {
			 msg = sendSMTP();
           }
		 else  if ("G".equalsIgnoreCase((String)client.get_Value("OAuthProvider")) || "M".equalsIgnoreCase((String)client.get_Value("OAuthProvider"))) {
			 msg = sendOAuth(client);
           }
		return msg;
...
//				props.put("mail.smtp.port", "587");
	...
	public String sendOAuth(MClient client) {
	    HttpURLConnection conn = null;
	    try {
	        // Step 1: Setup mail session
	        Properties props = new Properties();
	        Session session = Session.getInstance(props, null);
	        m_message = new MimeMessage(session);

	        // Step 2: Email headers
	        m_message.setFrom(m_from);
	        m_message.setRecipients(Message.RecipientType.TO, getTos());
	        if (getCcs() != null)
	            m_message.setRecipients(Message.RecipientType.CC, getCcs());
	        if (getBccs() != null)
	            m_message.setRecipients(Message.RecipientType.BCC, getBccs());
	        if (m_replyTo != null)
	            m_message.setReplyTo(new Address[]{m_replyTo});
	        m_message.setSentDate(new java.util.Date());
	        m_message.setHeader("Comments", "Sent via OAuth Mail API");

	        String charSetName = Ini.getCharset() != null ? Ini.getCharset().name() : "UTF-8";
	        m_message.setSubject(getSubject(), charSetName);

	        Multipart multipart = new MimeMultipart();

	        MimeBodyPart messagePart = new MimeBodyPart();
	        if (m_messageHTML != null && !m_messageHTML.isEmpty()) {
	            messagePart.setContent(m_messageHTML, "text/html; charset=" + charSetName);
	        } else {
	            messagePart.setText(getMessageCRLF(), charSetName);
	        }
	        ...
	        if (m_attachments != null && !m_attachments.isEmpty()) {
	            for (Object attachment : m_attachments) {
	                ...
	                if (attachment instanceof File) {
	                    ...
	                    ds = new URLDataSource(uri.toURL());
	                } else if (attachment instanceof DataSource) {
	                    ds = (DataSource) attachment;
	                } else {
	                    log.warning("Unknown attachment type: " + attachment);
	                    continue;
	                }
	        ...
	        if ("G".equalsIgnoreCase((String) client.get_Value("OAuthProvider"))) {
	            ...
	            accessToken = GetOAuthGoogleToken.getAccessToken(client);
	            ByteArrayOutputStream buffer = new ByteArrayOutputStream();
		        m_message.writeTo(buffer);
	            rawMessage = Base64.getUrlEncoder().withoutPadding().encodeToString(buffer.toByteArray());
	            jsonBody = "{ \"raw\": \"" + rawMessage + "\" }";
	        }
	        else if ("M".equalsIgnoreCase((String) client.get_Value("OAuthProvider"))) {
                url = new URL("https://graph.microsoft.com/v1.0/me/sendMail");
	            accessToken = GetOAuthMicrosoftToken.getAccessToken(client);
	            StringBuilder jsonBodyBuilder = new StringBuilder();
		        jsonBodyBuilder.append("{");
		        jsonBodyBuilder.append("\"message\": {");
		        jsonBodyBuilder.append("\"subject\": \"").append(escapeJson(getSubject())).append("\",");
		        jsonBodyBuilder.append("\"body\": {");
		        jsonBodyBuilder.append("\"contentType\": \"")
		                .append(m_messageHTML != null ? "HTML" : "Text")
		                .append("\",\"content\": \"")
		                .append(escapeJson(m_messageHTML != null ? m_messageHTML : getMessageCRLF()))
		                .append("\"},");
		        jsonBodyBuilder.append("\"toRecipients\": [").append(getFormattedToRecipients()).append("]");
		        jsonBodyBuilder.append("},\"saveToSentItems\": true}");

		        jsonBody = jsonBodyBuilder.toString();
	        }

	        if (accessToken == null || accessToken.trim().isEmpty()) {
	            return "SEND_FAILED: Access token is missing in AD_Client";
	        }
...
//	        String jsonBody = "{ \"raw\": \"" + rawMessage + "\" }";
	        ...
	        if (responseCode == 200 || responseCode == 202) {
	            ...
	            if (errorStream != null) {
	                ...
	                int bytesRead;
	                while ((bytesRead = errorStream.read(bufferErr)) != -1) {
	                    sb.append(new String(bufferErr, 0, bytesRead, StandardCharsets.UTF_8));
	                }
	                error = sb.toString();
	            }
	            return "SEND_FAILED: HTTP " + responseCode + " - " + error;
	        }

	    } catch (Exception e) {
	        log.log(Level.SEVERE, "OAuth Mail API send failed", e);
	        return "SEND_EXCEPTION: " + e.getMessage();
	    } finally {
	        if (conn != null) {
	            conn.disconnect();
	        }
	...
	private String getFormattedToRecipients() {
	    StringBuilder sb = new StringBuilder();
	    boolean first = true;
	    ...
	    for (Address address : tos) {
	        ...
	        sb.append("{")
	          .append("\"emailAddress\": {")
	          .append("\"address\": \"").append(escapeJson(ia.getAddress())).append("\"");

	        if (ia.getPersonal() != null && !ia.getPersonal().trim().isEmpty()) {
	            sb.append(", \"name\": \"").append(escapeJson(ia.getPersonal())).append("\"");
	        }
	...
	public String getAccessToken(String clientId, String clientSecret, String refreshToken) throws IOException {
	    ...
	    String accessToken = null;
	    try {
			 accessToken = extractAccessToken(json);
		} catch (Exception e) {
	...
	public String extractAccessToken(String json) throws Exception {
	    String key = "\"access_token\"";
	    int keyIndex = json.indexOf(key);
	    if (keyIndex == -1) {
	        throw new Exception("access_token not found in JSON: " + json);
	    }

	    int colonIndex = json.indexOf(":", keyIndex);
	    if (colonIndex == -1) {
	        throw new Exception("Colon not found after access_token");
	    }

	    // Skip whitespace after colon
	    int quoteStart = colonIndex + 1;
	    while (quoteStart < json.length() && Character.isWhitespace(json.charAt(quoteStart))) {
	        quoteStart++;
	    }

	    if (json.charAt(quoteStart) != '\"') {
	        throw new Exception("access_token value does not start with a quote");
	    }

	    int valueStart = quoteStart + 1;
	    int valueEnd = json.indexOf("\"", valueStart);
	    if (valueEnd == -1) {
	        throw new Exception("access_token value not properly closed with quote");
	    }
