...
import org.cyprusbrs.framework.I_AD_PInstance_Log;
import org.cyprusbrs.framework.MPInstance;
import org.cyprusbrs.framework.MPInstanceLog;
...
public class PInstanceLogElement extends GridElement {
	...
	private static int calculateRowCount(MQuery query, Properties ctx) {
		int AD_PInstance_ID = query.getAD_PInstance_ID();
		if (AD_PInstance_ID > 0) {
			String sql = "SELECT COUNT(*) FROM "+I_AD_PInstance_Log.Table_Name
							+" WHERE "+I_AD_PInstance_Log.COLUMNNAME_AD_PInstance_ID+"=?";
