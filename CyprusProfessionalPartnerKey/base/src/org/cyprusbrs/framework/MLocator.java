...
public class MLocator extends X_M_Locator
{
	...
	public boolean isFixed()
	{
	 ...
	 public static MLocator getDefaultLocatorOfOrg(Properties ctx, int AD_Org_ID) {
		    PreparedStatement cPreparedStatement = null;
		    MLocator retValue = null;
		    ...
		    try {
		      ...
		      while (rs != null && rs.next()) {
		        if (rs.getString(2) == "Y") {
		          defaultlocators.add(Integer.valueOf(rs.getInt(1)));
		          break;
		        } 
		        locators.add(Integer.valueOf(rs.getInt(1)));
		      } 
		      if (defaultlocators.size() > 0) {
		        retValue = get(ctx, Integer.valueOf(defaultlocators.get(0).intValue()).intValue());
		        return retValue;
		      } 
		      if (locators.size() > 0) {
		        retValue = get(ctx, Integer.valueOf(locators.get(0).intValue()).intValue());
		        return retValue;
		      } 
		    } catch (Exception e) {
		      s_log.log(Level.SEVERE, sql, e);
