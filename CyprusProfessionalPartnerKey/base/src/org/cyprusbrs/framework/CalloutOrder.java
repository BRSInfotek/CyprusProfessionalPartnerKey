		...
		if (Env.isSOTrx(ctx, WindowNo))
		{
			...
			if (product.isStocked())
			{
				...
				BigDecimal available = MStorage.getQtyAvailable
					(M_Warehouse_ID, M_Product_ID.intValue(), M_AttributeSetInstance_ID, trx);
				if (available == null)
					available = Env.ZERO;
...
//		int C_UOM_To_ID = Env.getContextAsInt(ctx, WindowNo, "C_UOM_ID");
		int C_UOM_To_ID =(Integer)mTab.getValue(MLeadInfo.COLUMNNAME_C_UOM_ID);   // Env.getContextAsInt(ctx, WindowNo, MLeadInfo.COLUMNNAME_C_UOM_ID);	
		...
		if(C_Order_ID>0 && M_Product_ID>0) // for Order
		{
			BigDecimal lineNetAmt=(BigDecimal)mTab.getValue("LineNetAmt");
			Integer taxId=UtilTax.getTaxIDBasedOnState(MOrder.Table_ID,C_Order_ID,M_Product_ID,lineNetAmt,PriceEntered);
...
//					(M_Warehouse_ID, M_Product_ID, M_AttributeSetInstance_ID, null);
				BigDecimal available = MStorage.getQtyAvailable(M_Warehouse_ID, M_Product_ID, M_AttributeSetInstance_ID, trx);
				if (available == null)
					available = Env.ZERO;
				if (available.signum() == 0)
					mTab.fireDataStatusEEvent ("NoQtyAvailable", "0", false);
	...
	//path: org.cyprusbrs.model.CalloutOrder.getDocSubTypeSO
		public String getDocSubTypeSO (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
		{
			Integer C_DocTypeTarget_ID = (Integer)value;
			if (C_DocTypeTarget_ID == null || C_DocTypeTarget_ID.intValue() == 0)
				return "";
			
			String docSubTypeSO = DB.getSQLValueString(null, "SELECT DocSubTypeSO FROM C_DocType WHERE C_DocType_ID=?", C_DocTypeTarget_ID);
			mTab.setValue("DocSubTypeSO", docSubTypeSO);
			return "";
			
		}	//	getDocSubTypeSO
		
		//path: org.cyprusbrs.model.CalloutOrder.setTaxBasedOnState
		public String setTaxBasedOnState(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) 
		{
		    ...
		    taxId=UtilTax.getTaxIDBasedOnState(MOrder.Table_ID,C_Order_ID,M_Product_ID,lineNetAmt, price);
		...
		//path: org.cyprusbrs.model.CalloutOrder.updateTaxBasedOnCharge
		...
		public String getTCSAmt (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
		{
			...
			MOrder order = new MOrder(ctx, C_Order_ID, null);
			String docBaseType = DB.getSQLValueString(null, "SELECT DocBaseType FROM C_DocType WHERE C_DocType_ID=?",new Object[] { Integer.valueOf(order.getC_DocTypeTarget_ID()) });
			if(m_product_id > 0 && ("SOO".equalsIgnoreCase(docBaseType)))
			{
				...
				int C_ConversionType_ID = order.getC_ConversionType_ID();
				Timestamp ConvDate = order.getDateOrdered();
