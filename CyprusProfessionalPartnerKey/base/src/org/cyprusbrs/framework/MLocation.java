...
public class MLocation extends X_C_Location implements Comparator
{
	...
	/**
	 * get existing location with all fields of location
	 * @param ctx
	 * @param C_Country_ID
	 * @param regionName
	 * @param C_Region_ID
	 ...
	 * @return
	 */
	public static MLocation getLocationToCheckAllFields (Properties ctx, int C_Country_ID, String regionName, int C_Region_ID, String postalCode,
			String cityName, int C_City_ID, String Address1, String Address2, String Address3, String Address4, String trxName)
	{
		if (C_Country_ID == 0)					//	load default
			return null;
		...
		if(postalCode!=null && postalCode.length()>0)
			sqlSB.append(" AND UPPER(POSTAL)='"+postalCode.toUpperCase()+"'");
		if(Address1!=null && Address1.length()>0)
			sqlSB.append(" AND UPPER(ADDRESS1)='"+Address1.toUpperCase()+"'");
		if(Address1!=null && Address2.length()>0)
			sqlSB.append(" AND UPPER(ADDRESS2)='"+Address2.toUpperCase()+"'");
		if(Address1!=null && Address3.length()>0)
			sqlSB.append(" AND UPPER(ADDRESS3)='"+Address3.toUpperCase()+"'");
		if(Address1!=null && Address4.length()>0)
			sqlSB.append(" AND UPPER(ADDRESS4)='"+Address4.toUpperCase()+"'");
		
		System.out.println(sqlSB);
		String sql = "SELECT * FROM C_Location l "
				+ "WHERE C_COUNTRY_ID=? AND "
				+ "(UPPER(REGIONNAME)=UPPER(?) OR C_REGION_ID=?) AND " // + "(UPPER(REGIONNAME)=UPPER('Uttar Pradesh') OR C_REGION_ID=128)"
				+ "(UPPER(CITY)=UPPER(?) OR C_CITY_ID=?) " // + "(UPPER(REGIONNAME)=UPPER('Uttar Pradesh') OR C_REGION_ID=128)"
				+ sqlSB.toString();
		...
		try
		{
			...
			pstmt.setString(2, regionName.toUpperCase());
			pstmt.setInt(3, C_Region_ID);
			pstmt.setString(4, cityName.toUpperCase());
			pstmt.setInt(5, C_City_ID);
			 rs = pstmt.executeQuery();
			if (rs.next())
				loc = new MLocation (ctx, rs, trxName);
			rs.close();
		...
		{
			s_log.log(Level.SEVERE, sql + " - " + C_Country_ID, e);
			loc = null;
		}
		finally
		{
	...
	public String toString()
	{
		...
		else
		{
			...
			retStr.append(", ").append(getCountryName()+"("+getCountry().getCountryCode()+")"); // Added by Mukesh @20230220
		}
	...
	/**	Return Exportable String representation
	 *  @return String
	 *  @author Mukesh Date: 20230221
	 */
	public String toStringExport()
	{
		...
		/// Check City ID first
		if(getC_City_ID()>0)
			retStr.append(", ").append((new MCity(getCtx(), getC_City_ID(), get_TableName()).getName()));
		else if (getCity() != null && getCity().length() > 0)
			retStr.append(", ").append(getCity());
		else
			retStr.append(", ");
		
		// Check Region ID first then check Region Name 
		if(getC_Region_ID()>0)
		{
			retStr.append(", ").append((new MRegion(getCtx(), getC_Region_ID(), get_TableName()).getName()));
		}
		else if (getRegionName() != null && getRegionName().length() > 0)
			retStr.append(", ").append(getRegionName());
		else
			retStr.append(", ");
		
		if (getPostal() != null && getPostal().length() > 0)
			retStr.append(", ").append(getPostal());
		else
			retStr.append(", ");
		
		if (getCountryName() != null && getCountryName().length() > 0)
			retStr.append(", ").append(getCountryName()+"("+getCountry().getCountryCode()+")");		//	City, Region, Postal
		else
			retStr.append(", ");
