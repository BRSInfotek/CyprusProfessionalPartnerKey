...
public class MAlertRule extends X_AD_AlertRule
{
	...
	private boolean updateParent() {
		final String sql_count = "SELECT COUNT(*) FROM "+Table_Name+" r"
							+" WHERE r."+COLUMNNAME_AD_Alert_ID+"=a."+I_AD_Alert.COLUMNNAME_AD_Alert_ID
								+" AND r."+COLUMNNAME_IsValid+"='N'"
								+" AND r.IsActive='Y'"
		;
		final String sql = "UPDATE "+I_AD_Alert.Table_Name+" a SET "
			+" "+I_AD_Alert.COLUMNNAME_IsValid+"=(CASE WHEN ("+sql_count+") > 0 THEN 'N' ELSE 'Y' END)"
			+" WHERE a."+I_AD_Alert.COLUMNNAME_AD_Alert_ID+"=?"
		;
