...
public class MProduct extends X_M_Product
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// code updated by Anshul @20240624
		
		Integer count= DB.getSQLValueEx(get_TrxName(),"SELECT COUNT(*) FROM C_TaxRule WHERE C_TaxCategory_ID = ?", getC_TaxCategory_ID());
		if (count > 0)
		{
			setIsTaxRule(true);
		}
		...
		// Check and update Resource Type Group by Mukesh @20250113 
		String rg=get_ValueAsString("ResourceGroup");
		if(newRecord && "P".equalsIgnoreCase(rg))
			setProductType(X_M_Product.PRODUCTTYPE_Resource);
		// End of the code
	...
	public boolean isASIMandatory(boolean isSOTrx) {
		...
		for (MAcctSchema as : mass)
		{
			...
			if (X_C_AcctSchema.COSTINGLEVEL_BatchLot.equals(cl)) {
				return true;
			}
		}
		//
