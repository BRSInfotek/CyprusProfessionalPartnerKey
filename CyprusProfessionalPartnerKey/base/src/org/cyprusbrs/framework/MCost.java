...
//		}
	...
	public static MCost[] get(int M_AttributeSetInstance_ID, MAcctSchema as, int M_CostType_ID, int AD_Org_ID, MProduct product) throws Exception {
	    CPreparedStatement cPreparedStatement = null;
	    String CostingLevel = as.getCostingLevel();
	    MProductCategoryAcct pca = MProductCategoryAcct.get(product.getCtx(), product.getM_Product_Category_ID(), as.getC_AcctSchema_ID(), null);
	    if (pca == null)
	      throw new Exception("Cannot find Acct for M_Product_Category_ID=" + product.getM_Product_Category_ID() + ", C_AcctSchema_ID=" + as.getC_AcctSchema_ID()); 
	    ...
	    String sql = "SELECT c.* FROM M_Cost c  LEFT OUTER JOIN M_CostElement ce ON (c.M_CostElement_ID=ce.M_CostElement_ID) WHERE c.AD_Client_ID=? AND c.AD_Org_ID=? AND c.M_Product_ID=? AND (c.M_AttributeSetInstance_ID=? OR c.M_AttributeSetInstance_ID=0)  AND c.M_CostType_ID=?  AND (ce.CostingMethod IS NULL OR ce.CostingMethod=?)  AND c.IsActive = 'Y' ";
	    if (as != null)
	      sql = sql + " AND C_AcctSchema_ID=?"; 
	    PreparedStatement pstmt = null;
	    ResultSet rs = null;
	    try {
	      cPreparedStatement = DB.prepareStatement(sql, product.get_TrxName());
	      cPreparedStatement.setInt(1, product.getAD_Client_ID());
	      cPreparedStatement.setInt(2, AD_Org_ID);
	      cPreparedStatement.setInt(3, product.getM_Product_ID());
	      cPreparedStatement.setInt(4, M_AttributeSetInstance_ID);
	      if (M_CostType_ID == 0) {
	        cPreparedStatement.setInt(5, as.getM_CostType_ID());
	      } else {
	        cPreparedStatement.setInt(5, M_CostType_ID);
	      } 
	      cPreparedStatement.setString(6, X_M_CostElement.COSTINGMETHOD_StandardCosting);
	      if (as != null)
	        cPreparedStatement.setInt(7, as.getC_AcctSchema_ID()); 
