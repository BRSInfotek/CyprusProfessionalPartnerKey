...
public class MMovement extends X_M_Movement implements DocAction
{
	...
	public MMovementLine[] getLines (boolean requery)
	{
		...
		List<MMovementLine> list = new Query(getCtx(), I_M_MovementLine.Table_Name, whereClause, get_TrxName())
		 										...
		 										.setOrderBy(I_M_MovementLine.COLUMNNAME_Line)
		 										.list();
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		{
			MDocType types[] = MDocType.getOfDocBaseType(getCtx(), X_C_DocType.DOCBASETYPE_MaterialMovement);
	...
	public String completeIt()
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
					...
					&& product.isStocked() )
			{
				...
				MLocator locatorTo = new MLocator (getCtx(), line.getM_LocatorTo_ID(), get_TrxName()); //As discussion with Anshul updated by Mukesh @20240829
				...
				if (line.getM_AttributeSetInstance_ID() == 0)
				{
					...
					for (int j = 0; j < mas.length; j++)
					{
						...
						//
						MTransaction trxTo = new MTransaction (getCtx(), locatorTo.getAD_Org_ID(), // line.getAD_Org_ID(), As discussion with Anshul updated by Mukesh @20240827 
						...
						// Updated by Mukesh @20240709 regarding costing while complete button
				...
				if (trxFrom == null)
				{
					...
					{	
						String err = createCostDetail(line,line.getM_AttributeSetInstance_ID(), line.getMovementQty());
						if (err != null && err.length() > 0) {
							m_processMsg = err;
							return DocAction.STATUS_Invalid;
						}
			...
			/**
			 * Code updated by Mukesh @20250227
			 */
			// SQL to update QtyTransferred in Stock Transfer Requisition Line
			String updateSQL = "UPDATE M_RequisitionLine " +
			                   "SET QtyTransferred = ? " +
			                   "WHERE M_RequisitionLine_ID=?";

			// Execute the update
			int updatedRows = DB.executeUpdate(updateSQL, new Object[]{line.getMovementQty(), line.get_Value("M_RequisitionLine_ID")}, false, get_TrxName());

			if (updatedRows > 0) {
			    log.info("Updated QtyTransferred for Requisition Line ID: " + line.get_Value("M_RequisitionLine_ID"));
			} else {
			    log.info("No update performed. Check if movement exists for Requisition Line ID: " + line.get_Value("M_RequisitionLine_ID"));
			}
	 ...
	 * 
	 */
	private String createCostDetail(MMovementLine line, int M_AttributeSetInstance_ID ,BigDecimal qty)
	{
		...
		// update TrxCost @Mukesh 20240716
			...
			{
				// @ M_MovementLine
				int retVal=DB.executeUpdate("UPDATE M_MovementLine SET TrxCost="+unitCost+" WHERE M_MovementLine_ID="+line.getM_MovementLine_ID(), get_TrxName() );
				 ...
				 */
				// @ M_Transaction
				retVal=DB.executeUpdate("UPDATE M_Transaction SET TrxCost="+unitCost+" WHERE M_MovementLine_ID="+line.getM_MovementLine_ID(), get_TrxName() );
...
//			Only for between-org movements
			...
			if(true) // (fromLocator.getAD_Org_ID() != toLocator.getAD_Org_ID())
			{
				...
				if (description == null)
					description = "";
				//	Cost Detail From
				MCostDetail.createMovement(as, fromLocator.getAD_Org_ID(), 	//	from locator org
						line.getM_Product_ID(), M_AttributeSetInstance_ID,
						line.get_ID(), 0,
						costs.negate(), qty.negate(), true,
						description + "(Out|->)", get_TrxName());
				//	Cost Detail To
				MCostDetail.createMovement(as, toLocator.getAD_Org_ID(),	//	to locator org 
						line.getM_Product_ID(), M_AttributeSetInstance_ID,
						line.get_ID(), 0,
						costs, qty, false,
...
//		    	BigDecimal currentCostDT = MCostQueue.getCosts(product, M_AttributeSetInstance_ID, as, getAD_Org_ID(), ce, qty.negate(), null);
			    MCostDetail cd = MCostDetail.get (as.getCtx(), "M_MovementLine_ID=?", 
						line.getM_MovementLine_ID(), M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), get_TrxName());
	 ...
	 * @return
	 */
	public BigDecimal getCost(MMovementLine line, MAcctSchema as)
	{
		MMovementLineMA[] m_linesma = MMovementLineMA.get(getCtx(), line.getM_MovementLine_ID(), get_TrxName());
		BigDecimal cost = Env.ZERO;
		BigDecimal qty = Env.ZERO;

		for(int ma = 0; ma < m_linesma.length; ma++)
		{
			MMovementLineMA lineMA = m_linesma[ma];
