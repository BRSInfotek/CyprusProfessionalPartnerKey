...
public class MRequisition extends X_M_Requisition implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// Fetch the Document Base Type using SQL query
        String docBaseType = DB.getSQLValueString(get_TrxName(),
            "SELECT DocBaseType FROM C_DocType WHERE C_DocType_ID = ?", getC_DocType_ID());

        // Apply validation only if DocBaseType is 'STR' (Stock Transfer Requisition)
        if ("STR".equals(docBaseType)) {
            int fromWarehouse = getM_Warehouse_ID(); //get_ValueAsInt("M_Warehouse_ID");
            int toWarehouse =  get_ValueAsInt("M_WarehouseTo_ID");

            // Ensure From Warehouse and To Warehouse are different
            if (fromWarehouse == toWarehouse) {
                log.saveError("Error", Msg.getMsg(getCtx(), "From Warehouse and To Warehouse must be different!"));
