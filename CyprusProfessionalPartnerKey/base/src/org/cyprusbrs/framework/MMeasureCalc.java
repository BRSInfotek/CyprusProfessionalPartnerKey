...
public class MMeasureCalc extends X_PA_MeasureCalc
{
		...
		String MeasureScope, String MeasureDataType, Timestamp reportDate, MRole role)
	{
		...
		if (getDateColumn() != null 
			&& X_PA_Measure.MEASUREDATATYPE_QtyAmountInTime.equals(MeasureDataType)
...
//		}
		...
		String tableName, String orgColumn, String bpColumn, String pColumn)
	{
		...
		if (orgColumn != null)
		{
			...
			{
				if (X_PA_GoalRestriction.GOALRESTRICTIONTYPE_Organization.equals(restrictions[i].getGoalRestrictionType()))
		...
		if (bpColumn != null)
		{
			...
			for (int i = 0; i < restrictions.length; i++)
			{
				if (X_PA_GoalRestriction.GOALRESTRICTIONTYPE_BusinessPartner.equals(restrictions[i].getGoalRestrictionType()))
					listBP.add(restrictions[i].getC_BPartner_ID());
				//	Hierarchy comes here
				if (X_PA_GoalRestriction.GOALRESTRICTIONTYPE_BusPartnerGroup.equals(restrictions[i].getGoalRestrictionType()))
		...
		if (pColumn != null)
		{
			...
			for (int i = 0; i < restrictions.length; i++)
			{
				if (X_PA_GoalRestriction.GOALRESTRICTIONTYPE_Product.equals(restrictions[i].getGoalRestrictionType()))
					listP.add(restrictions[i].getM_Product_ID());
				//	Hierarchy comes here
				if (X_PA_GoalRestriction.GOALRESTRICTIONTYPE_ProductCategory.equals(restrictions[i].getGoalRestrictionType()))
