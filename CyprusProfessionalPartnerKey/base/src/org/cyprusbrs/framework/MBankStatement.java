...
//		}
	...
	if(is_ValueChanged(I_C_BankStatement.COLUMNNAME_StatementDate))
	{
		...
		{
			Optional<MDocType> doctypeOptional = Arrays.stream(MDocType.getOfDocBaseType(getCtx(), X_C_DocType.DOCBASETYPE_BankStatement))
	...
	public String prepareIt()
	{
		...
		MPeriod.testPeriodOpen(getCtx(), minDate, X_C_DocType.DOCBASETYPE_BankStatement, 0);
		MPeriod.testPeriodOpen(getCtx(), maxDate, X_C_DocType.DOCBASETYPE_BankStatement, 0);
	...
	static public MBankStatementLine addPayment(MPayment payment)
	{
		...
		whereClause.append(I_C_BankStatement.COLUMNNAME_C_BankAccount_ID).append("=? AND ")
				.append("TRUNC(").append(I_C_BankStatement.COLUMNNAME_StatementDate).append(",'DD')=? AND ")
				.append(I_C_BankStatement.COLUMNNAME_Processed).append("=?");
		MBankStatement bankStatement = new Query(payment.getCtx() , I_C_BankStatement.Table_Name , whereClause.toString(), payment.get_TrxName())
				.setClient_ID()
