...
//				String costingMethod = product.getCostingMethod(as);
			...
			// code update by Anshul @21072024 to get cost from Product Costs
			MCostDetail cd = MCostDetail.get (as.getCtx(), "C_OrderLine_ID=?", 
					oLine.getC_OrderLine_ID(), getM_AttributeSetInstance_ID(), as.getC_AcctSchema_ID(), get_TrxName());
			BigDecimal costs = MCost.getCurrentCost(product, oLine.getM_AttributeSetInstance_ID(), as, oLine.getAD_Org_ID(), costingMethod, tQty, 0, true, get_TrxName());
			// code update by Anshul @19072024 to Update Transaction Cost on Line and Transaction tab on product
			BigDecimal trxCost = Env.ZERO;
			if(X_C_DocType.DOCBASETYPE_MaterialReceipt.equals(docBaseType) && !dt.isReturnTrx())
			{
				// 
				if(!as.isPostPOVariance() || X_M_Product_Category_Acct.COSTINGMETHOD_Lifo.equals(costingMethod) || X_M_Product_Category_Acct.COSTINGMETHOD_Fifo.equals(costingMethod))
				{						
					trxCost = oLine.getTrxCost();
				}
				else
				{	
					if(costs != null && tQty !=null)
					trxCost = costs.divide(tQty);
				}
			}
			
			// update trx cost on Material Receipt Line
			int updateInOut = DB.executeUpdateEx("UPDATE M_InOutLine SET TrxCost=" + trxCost.abs() + " WHERE M_InOutLine_ID=" + receiptLine.getM_InOutLine_ID(), get_TrxName());
			log.saveInfo("", "" + updateInOut);
		    // update trx cost on transaction tab of product
			int updateTrxCost = DB.executeUpdateEx("UPDATE M_Transaction SET TrxCost=" + trxCost.abs() + " WHERE M_InOutLine_ID=" + receiptLine.getM_InOutLine_ID(), get_TrxName());
			log.saveInfo("", "" + updateTrxCost);				
			// code update by Anshul @29072024 to set current cost price on cost detail
			if(cd != null)
			{
			...
			// update by Anshul @20240723 to set Transaction cost and PO cost on Matched purchase order
			setTrxCost(trxCost.abs());
			setPOCost(oLine.getTrxCost());
		}
	}
	return "";
}
