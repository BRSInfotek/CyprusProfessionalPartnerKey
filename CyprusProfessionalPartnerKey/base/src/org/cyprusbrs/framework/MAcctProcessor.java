...
//		int no = 0;// DB.executeUpdate(sql, get_TrxName()); /// Commented regarding to check DB Connection issue @20230309 by Mukesh
//		return no;
//	}	//	deleteLog
	...
	public int deleteLog()
	{
		...
		MRole.getDefault(getCtx(),true).addAccessSQL(COLUMNNAME_AD_Org_ID, COLUMNNAME_AD_Client_ID, isMandatoryOK(), isActive());
		int no = 0;
//		if (getKeepLogDays() < 1)
//			return no;
//		String sqlmax = "select count(*) from C_AcctProcessorLog where C_AcctProcessor_ID=" + getC_AcctProcessor_ID() +" and created < (sysdate-" + getKeepLogDays() + ")";
//		int count = DB.getSQLValueEx(get_TrxName(), sqlmax);
//		log.info("No. of count for account processor = " + count );
//		if(count > 1400)
//		{
//			String sql = "DELETE C_AcctProcessorLog"
//					+ " where C_AcctProcessor_ID=" + getC_AcctProcessor_ID() + " and  created < (sysdate-" + getKeepLogDays() + ")";				
//		 no = DB.executeUpdateEx(sql, get_TrxName());
//		 log.info("No. of delete records for account processor = " + no);
//		}
//		else
//		{
//			no = 0;
