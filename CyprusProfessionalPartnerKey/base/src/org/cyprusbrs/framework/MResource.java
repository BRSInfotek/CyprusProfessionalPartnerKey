...
public class MResource extends X_S_Resource
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		if (newRecord)
		{
			if("PN".equalsIgnoreCase(getManufacturingResourceType()))
			{
				String sql = "SELECT M_Product_ID FROM M_Product WHERE UPPER(Value) = ?";
			    Integer M_Product_ID = DB.getSQLValue(get_TrxName(), sql, getName().toUpperCase());

			    if (M_Product_ID != null && M_Product_ID > 0) {
			        log.saveError("DuplicateProduct", Msg.getMsg(getCtx(), "SaveProductError"));
			        return false;
			    }
				...
				// Add extra details from Manufacturing Resource window by Mukesh @20250702
				m_product.setC_TaxCategory_ID(getC_TaxCategory_ID());
				m_product.setC_Tax_ID(getC_Tax_ID());
				m_product.set_ValueNoCheck("Ref_C_Tax_ID", getRef_C_Tax_ID());
				m_product.setM_Product_Category_ID(105);  // Standard
				m_product.setC_UOM_ID(101); // Hour
				m_product.save();
