	...
	implements CyprusProcessor
{
	...
	public int deleteLog1()
	{
		int no = 0;
		if (getKeepLogDays() < 1)
			return no;
		String sqlmax = "select count(*) from AD_AlertProcessorLog where AD_AlertProcessor_ID=" + getAD_AlertProcessor_ID() +" and created < (sysdate-" + getKeepLogDays() + ")";
		int count = DB.getSQLValueEx(get_TrxName(), sqlmax);
		log.info("No. of count for alert processor = " + count );
		if(count > 15)
		{
			String sql = "DELETE AD_AlertProcessorLog"
					+ " where AD_AlertProcessor_ID=" + getAD_AlertProcessor_ID() + " and  created < (sysdate-" + getKeepLogDays() + ")";				
		 no = DB.executeUpdateEx(sql, get_TrxName());
		 log.info("No. of delete records for alert processor = " + no);
		}
		else
		{
			no = 0;
