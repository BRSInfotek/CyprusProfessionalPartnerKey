...
public class MMatchInv extends X_M_MatchInv
{
	...
	protected boolean beforeDelete ()
	{
		if (isPosted())
		{
			MPeriod.testPeriodOpen(getCtx(), getDateTrx(), X_C_DocType.DOCBASETYPE_MatchInvoice, getAD_Org_ID());
			setPosted(false);
	...
	private String createMatchInvCostDetail()
	{
		...
		for(int asn = 0; asn < acctschemas.length; asn++)
		{
			...
			if (receipt.getMovementType().equals(X_M_InOut.MOVEMENTTYPE_VendorReturns))
			...
			// code update by Anshul @29072024 to update current cost on cost detail
			BigDecimal trxCost = Env.ZERO;
			if(invoiceLine.getM_InOutLine_ID() > 0)
			{
				MInOutLine iol = new MInOutLine(getCtx(), invoiceLine.getM_InOutLine_ID(), get_TrxName());
				trxCost = iol.getTrxCost();
			}
			else if(invoiceLine.getC_OrderLine_ID() > 0)
			{
				MOrderLine ol = new MOrderLine(getCtx(), invoiceLine.getC_OrderLine_ID(), null);
				trxCost = ol.getTrxCost();
			}
			
			MCostDetail cd = MCostDetail.get (as.getCtx(), "C_InvoiceLine_ID=?", 
					invoiceLine.getC_InvoiceLine_ID(), getM_AttributeSetInstance_ID(), as.getC_AcctSchema_ID(), get_TrxName());
			if(cd != null && trxCost.signum() != 0)
			{
				cd.setCurrentCostPrice(trxCost);
				cd.save();
			}		
		}
		...
		int M_InOutLine_ID, String trxName)
	{
		...
		//
		final String whereClause = I_M_MatchInv.COLUMNNAME_M_InOutLine_ID+"=?";
		List<MMatchInv> list = new Query(ctx, I_M_MatchInv.Table_Name, whereClause, trxName)
