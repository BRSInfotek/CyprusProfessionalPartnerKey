...
//		/// End of code by Mukesh @20210113
		...
		if ((newRecord || is_ValueChanged(I_C_BankStatementLine.COLUMNNAME_StmtAmt)))	//	allow different org for charge
			setStmtAmt(UtilPayment.getRoundingAmountFromStdPrecision(getStmtAmt(),getC_Currency_ID()));
		if((newRecord || is_ValueChanged(I_C_BankStatementLine.COLUMNNAME_TrxAmt)))
			setTrxAmt(UtilPayment.getRoundingAmountFromStdPrecision(getTrxAmt(),getC_Currency_ID()));
		if((newRecord || is_ValueChanged(I_C_BankStatementLine.COLUMNNAME_ChargeAmt)))
			setChargeAmt(UtilPayment.getRoundingAmountFromStdPrecision(getChargeAmt(),getC_Currency_ID()));
		if((newRecord || is_ValueChanged(I_C_BankStatementLine.COLUMNNAME_InterestAmt)))
		...
		if(getChargeAmt().compareTo(Env.ZERO) != 0 || is_ValueChanged(I_C_BankStatementLine.COLUMNNAME_C_Tax_ID))
		{
