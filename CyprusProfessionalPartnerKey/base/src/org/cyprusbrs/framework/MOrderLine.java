...
//		}
	 ...
	 * Updated by Mukesh @20241112... This method pick from Invoice Line 
	 */
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (!newRecord && is_ValueChanged("C_Tax_ID"))
		{
			...
			if (!getParent().isProcessed())
			{
				...
				{
					if (!updateOrderTaxForReverseCharge(true))
						return false;
				}
			}
		}
...
//		//	Recalculate Tax for this Tax
		if (!getParent().isProcessed())
		{
			...
			if("Y".equalsIgnoreCase(isReverseCharge))
			{
				if (!updateOrderTaxForReverseCharge(false))
					return false;
			}
		}
		...
		if (isTaxIncluded())
			sql = "UPDATE C_Order i "
				+ " SET GrandTotal=TotalLines + "
					+ "(SELECT COALESCE(SUM(TCSAmt),0) FROM C_OrderLine il WHERE i.C_Order_ID=il.C_Order_ID) "
				+ "WHERE C_Order_ID=" + getC_Order_ID();
		else
			sql = "UPDATE C_Order i "
				+ " SET GrandTotal=TotalLines+"
		...
		String sqlUpdateTCS = "UPDATE C_Order i"
				+ " SET TCSTotalAmt="
				+ "(SELECT COALESCE(SUM(TCSAmt),0) FROM C_OrderLine il WHERE i.C_Order_ID=il.C_Order_ID) "
				+ "WHERE C_Order_ID=?";
			int noUpdateTCS = DB.executeUpdateEx(sqlUpdateTCS, new Object[]{getC_Order_ID()}, get_TrxName());
			if (noUpdateTCS != 1)
	...
	private boolean updateOrderTaxForReverseCharge(boolean oldTax) 
	{
		String trxName = Trx.createTrxName("taxOrder");
		Trx trx = Trx.get(trxName, true);
		MOrderTax tax = MOrderTax.get (this, getPrecision(), oldTax, get_TrxName());
		int count = DB.getSQLValue(null, "SELECT COUNT(*) FROM C_OrderTax WHERE C_Order_ID=" + tax.getC_Order_ID() +
				" AND C_Tax_ID=" + tax.getC_Tax_ID() + " AND TaxAmt=" + tax.getTaxAmt().negate());
		if(count == 0)
		{
			MOrderTax taxNew = new MOrderTax(getCtx(), 0, trxName);
			if (tax != null) {
				taxNew.set_TrxName(trxName);
				taxNew.setClientOrg(this);
				taxNew.setC_Order_ID(getC_Order_ID());
