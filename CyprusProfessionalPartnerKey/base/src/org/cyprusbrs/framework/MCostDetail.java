...
public class MCostDetail extends X_M_CostDetail
{
		...
		String Description, boolean IsSOTrx, String trxName)
	{
		...
		MInOut header = IOL.getParent();
		MDocType dt = new MDocType(Env.getCtx(), header.getC_DocType_ID(), null);
		//	Delete Unprocessed zero Differences
		String sql = "DELETE M_CostDetail "
			+ "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0"
		...
		if (cd == null)		//	createNew
		{
			...
			{
				cd.set_ValueNoCheck("IsJobWorkMinus", "Y");
			}
			else
			{
	 ...
	 *	@return true if no error
	 */
	public static boolean createCostCollector (MAcctSchema as, int AD_Org_ID, 
		int M_Product_ID, int M_AttributeSetInstance_ID,
		int PP_Cost_CollectorLine_ID, int M_CostElement_ID, 
		BigDecimal Amt, BigDecimal Qty, boolean from,
		String Description, String trxName)
	{
		//	Delete Unprocessed zero Differences
		...
		if (no != 0)
			s_log.config("Deleted #" + no);
		MCostDetail cd = get (as.getCtx(), "PP_Cost_CollectorLine_ID=?", 
				PP_Cost_CollectorLine_ID, M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), trxName);
		//
		if (cd == null)		//	createNew
		{
			cd = new MCostDetail (as, AD_Org_ID, 
				...
				M_CostElement_ID, 
				Amt, Qty, Description, trxName);
			cd.set_ValueNoCheck("PP_Cost_CollectorLine_ID", PP_Cost_CollectorLine_ID);
	...
	//			as, Org_ID, ce.getM_CostElement_ID());
		...
		**/
		...
		{		
			...
			// code update by Anshul @05072024 Regarding update cost on Average Invoice on complete MR
			if (ce.isAverageInvoice())
			{
				cost.setWeightedAverage(amt, qty);
			...
			// code update by Anshul @17072024 Regarding update cost on Average Invoice on complete MR
			else if (ce.isFifo()
					|| ce.isLifo())
			{
...
//				// update by Anshul @23072024 to update qty , accumulate amt, accumulated qty on product costs if costing method null
...
//						&& cost.getCurrentCostPriceLL().signum() == 0)
		...
		else if (getC_InvoiceLine_ID() != 0)
		{
			...
			MOrderLine orderLine = new MOrderLine(getCtx(), invLine.getC_OrderLine_ID(), get_TrxName());
			MInOutLine il = new MInOutLine(getCtx(), invLine.getM_InOutLine_ID(), get_TrxName());
			// change by Anshul @05072024 to update cost on product cost if price change on invoice.		
		        if (ce.isAverageInvoice() && qty.signum() > 0)
				{
		        	int schemaPrecision = 0;
		    		if (as != null)
		    			schemaPrecision = as.getCostingPrecision();
		    		schemaPrecision = 6;
		    		BigDecimal qtyTest = qty;
//		        	MInvoiceLine invLine = new MInvoiceLine(getCtx(), getC_InvoiceLine_ID(), get_TrxName());
//					MOrderLine orderLine = new MOrderLine(getCtx(), invLine.getC_OrderLine_ID(), get_TrxName());
		        	if(invLine.getPriceActual().subtract(orderLine.getPriceActual()).compareTo(Env.ZERO)!= 0)
					{
						if(invLine.getQtyInvoiced().subtract(orderLine.getQtyOrdered()).compareTo(Env.ZERO)==0)
						{
							cost.setCumulatedAmt(amt);
							if(cost.getCumulatedQty().signum() > 0)
							{
								cost.setCurrentCostPrice(amt.divide(cost.getCumulatedQty(),schemaPrecision, BigDecimal.ROUND_HALF_UP));
							}
						}
						else 
						{		
							BigDecimal invoiceCost = (invLine.getPriceActual().subtract(orderLine.getPriceActual())).multiply(invLine.getQtyInvoiced()); 
							cost.setCumulatedAmt(cost.getCumulatedAmt().add(invoiceCost));
							if(cost.getCumulatedQty().signum() > 0)
							{
								cost.setCurrentCostPrice(cost.getCumulatedAmt().divide(cost.getCumulatedQty(),schemaPrecision, BigDecimal.ROUND_HALF_UP));
							}
						}
					}
				}
...
//				MInvoiceLine invLine = new MInvoiceLine(getCtx(), getC_InvoiceLine_ID(), get_TrxName());
//				MInOutLine il = new MInOutLine(getCtx(), invLine.getM_InOutLine_ID(), get_TrxName());
				if(il.getMovementQty().equals(cq.getCurrentQty()) && qty.signum() > 0)
				{
					cq.setCurrentCostPrice(amt.divide(qty,precision, BigDecimal.ROUND_HALF_UP));					
				}
				else
				{
					BigDecimal invoiceAmt = invLine.getPriceActual().multiply(invLine.getQtyEntered());
					BigDecimal cqAmt = cq.getCurrentCostPrice().multiply(cq.getCurrentQty());
					BigDecimal remainingQty = invLine.getQtyEntered().add(cq.getCurrentQty().negate());
					if(remainingQty.signum() > 0)
					{
						cq.setCurrentCostPrice((invoiceAmt.add(cqAmt.negate())).divide(remainingQty));
					}
				}
				BigDecimal amount = (invLine.getPriceActual().add(cq.getCurrentCostPrice().negate())).multiply(invLine.getQtyEntered());
...
//				cost.setCumulatedAmt(amt);
			...
			else if (ce.isStandardCosting() && invLine.getC_OrderLine_ID() == 0)
			{
				// Update cost record only if newly created.
				// Elsewhere we risk to set the CurrentCostPrice to an undesired price. 
//				if (cost.is_new()
...
////					}
//				}
//				else //original logic from cyprusbrs
//				{
//					// code update by Anshul @22072024 to calculate landed cost
////					BigDecimal oldSum = cost.getCurrentCostPrice().multiply(cost.getCurrentQty());
////					BigDecimal sumAmt = oldSum.add(amt);	//	amt is total already
////					BigDecimal costs = sumAmt.divide(qty.add(cost.getCurrentQty()), precision, BigDecimal.ROUND_HALF_UP);
//////					BigDecimal cCosts = cost.getCurrentCostPrice().add(amt);
////					cost.setCurrentCostPrice(costs);
////					cost.add(amt, qty);
//				}
				
				// code update by Anshul @25072024 	to calculate landed cost
				if(cost.getCurrentCostPrice().signum() == 0 && amt.signum() > 0)
				{
					if(cost.getCurrentQty().signum() > 0)
					{
						cost.setCurrentCostPrice(amt.divide(cost.getCurrentQty(), precision, BigDecimal.ROUND_HALF_UP));
						cost.setCumulatedAmt(amt);		
					}
										
				}
				else if(cost.getCurrentCostPrice().signum() > 0 && amt.signum() > 0)
				{
					BigDecimal oldSum = cost.getCurrentCostPrice().multiply(cost.getCurrentQty());
					...
					if(cost.getCurrentQty().signum() > 0)
					{
						BigDecimal costs = sumAmt.divide(cost.getCurrentQty(), precision, BigDecimal.ROUND_HALF_UP);
						cost.setCurrentCostPrice(costs);
						cost.setCumulatedAmt(cost.getCumulatedAmt().add(amt));
					}					
				}
		...
		else if (ce.isStandardCosting() && get_ValueAsInt("PP_Cost_CollectorLine_ID") != 0)
		{
//			if (cost.getCurrentCostPrice().signum() == 0
//					&& cost.getCurrentCostPriceLL().signum() == 0)
//			{
...
//			|| getPP_Cost_Collector_ID() != 0
			)
		{
			...
			// code update by Anshul @20240711 to re calculate cost in case of return to vendor.
			...
			{
				if (addition &&
						!((MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) || MDocType.DOCBASETYPE_MaterialReceipt.equals(docBaseType)) && dt.isReturnTrx()))
					cost.setWeightedAverage(amt, qty);
				else if((MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) || MDocType.DOCBASETYPE_MaterialReceipt.equals(docBaseType))  /* && !dt.isSOTrx() */ && dt.isReturnTrx())
				{
					BigDecimal oldAmount = cost.getCurrentCostPrice().multiply(cost.getCurrentQty());
					BigDecimal newCost = (oldAmount.add(amt)).divide(cost.getCurrentQty().add(qty), precision, BigDecimal.ROUND_HALF_UP);
					cost.setCurrentCostPrice(newCost);
					cost.setCurrentQty(cost.getCurrentQty().add(qty));
				}
				else
				cost.setCurrentQty(cost.getCurrentQty().add(qty));
			...
			else if (ce.isFifo() || ce.isLifo())
			{
				...
				{
					int attributeSetInstance_ID = M_ASI_ID;
					if((MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) || MDocType.DOCBASETYPE_MaterialReceipt.equals(docBaseType))  /* && !dt.isSOTrx() */ && dt.isReturnTrx())
					{
						attributeSetInstance_ID = inOutLine.getM_AttributeSetInstance_ID();
					}
					//	Adjust Queue - costing level Org/ASI
//					MCostQueue.adjustQty(product, M_ASI_ID, 
//						as, Org_ID, ce, qty.negate(), get_TrxName());
					MCostQueue.adjustQty(product, attributeSetInstance_ID, 
							as, Org_ID, ce, qty.negate(), get_TrxName());
...
//				MCostQueue[] cQueue = MCostQueue.getQueue(product, M_ASI_ID, 
//					as, Org_ID, ce, get_TrxName());
				MCostQueue[] cQueue = MCostQueue.getQueue(product, attributeSetInstance_ID, 
						as, Org_ID, ce, get_TrxName());				
				if (cQueue != null && cQueue.length > 0)
					cost.setCurrentCostPrice(cQueue[0].getCurrentCostPrice());
				// by Anshul @20250210
				if (getM_InventoryLine_ID() != 0 && amt.compareTo(Env.ZERO) > 0)
					cost.add(amt, qty); 
				else
					// End of the code
				cost.setCurrentQty(cost.getCurrentQty().add(qty));
			...
			{
			// Added by Mukesh @20240708 as suggested by Surya Last Invoice should also
				// update on Physical Inventory window
				if (getM_InventoryLine_ID() != 0 && addition && amt.compareTo(Env.ZERO) != 0) {
					if(qty.signum()!=0)
					{	
						cost.setCurrentCostPrice(amt.divide(qty));
						cost.add(amt, qty);
					}
				} else
					// End of the code
						...
						for (int i = 0 ; i < lce.length ; i++)
						{
							...
							if (lCost != null)
							{
								if (qtyOnhand.signum() != 0)
								{
									...
									if(!MDocType.DOCBASETYPE_MaterialDelivery.equals(dt.getDocBaseType()) && !dt.isReturnTrx())
									{
										lCost.setCurrentCostPrice(costs);	
									}
									// code update by Anshul @29072024
									if((MDocType.DOCBASETYPE_MaterialDelivery.equals(dt.getDocBaseType()) || MDocType.DOCBASETYPE_MaterialReceipt.equals(dt.getDocBaseType())) && dt.isReturnTrx())
									{
										lCost.setCurrentCostPrice(costs);
									}
								}
								lCost.setCurrentQty(qtyOnhand);
								// code update by anshul @26072024
								if(qty.signum() > 0 && !((MDocType.DOCBASETYPE_MaterialReceipt.equals(dt.getDocBaseType())) && dt.isReturnTrx()))
								{
									lCost.setCumulatedQty(lCost.getCumulatedQty().add(qty.divide(new BigDecimal(2))));
								}
								if (!lCost.save())
								{
		...
		/// Added by Mukesh @20240711 to calculate accumulated amount and qty for
		/// Movement only
		if (getM_MovementLine_ID() != 0 && qty.signum() != 0) {
			BigDecimal amount = (amt.divide(qty)).multiply(cost.getCurrentQty());
			setCumulatedAmt(amount);
			setCumulatedQty(cost.getCurrentQty());
			if(cost.getCurrentQty().signum()!=0)
			setCurrentCostPrice(amount.divide(cost.getCurrentQty(), precision, BigDecimal.ROUND_HALF_UP));
	...
	public static boolean createWorkOrderTransaction (MAcctSchema as, int AD_Org_ID, 
		int M_Product_ID, int M_AttributeSetInstance_ID,
		int MFG_WorkOrderTrxLine_ID, int M_CostElement_ID, 
		BigDecimal Amt, BigDecimal Qty,
		String Description, boolean IsSOTrx, String trx)
	{
		//	Delete Unprocessed zero Differences
		String sql = "DELETE FROM M_CostDetail "
			+ "WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0"
			+ " AND MFG_WorkOrderTrxLine_ID=" + MFG_WorkOrderTrxLine_ID
			+ " AND M_AttributeSetInstance_ID=" + M_AttributeSetInstance_ID
			+ " AND C_AcctSchema_ID = " + as.getC_AcctSchema_ID();
		int no = DB.executeUpdate(sql, trx);
		if (no != 0)
			s_log.config("Deleted #" + no);
		MCostDetail cd = get (as.getCtx(), "MFG_WorkOrderTrxLine_ID=? AND M_AttributeSetInstance_ID=?", 
				MFG_WorkOrderTrxLine_ID, M_AttributeSetInstance_ID, trx);
		//
		if (cd == null)		//	createNew
		{
			cd = new MCostDetail (as, AD_Org_ID, 
			...
			cd.setMFG_WorkOrderTrxLine_ID(MFG_WorkOrderTrxLine_ID);
			cd.setIsSOTrx(IsSOTrx);
		}
		else
		{
	...
	public static boolean createWorkOrderResourceTransaction(MAcctSchema as, int AD_Org_ID, int M_Product_ID,
			int M_AttributeSetInstance_ID, int MFG_WorkOrderResTxnLine_ID, int M_CostElement_ID, BigDecimal Amt,
			BigDecimal Qty, String Description, boolean IsSOTrx, String trxName) {
		String sql = "DELETE FROM M_CostDetail WHERE Processed='N' AND COALESCE(DeltaAmt,0)=0 AND COALESCE(DeltaQty,0)=0 AND MFG_WorkOrderResTxnLine_ID= ?  AND M_AttributeSetInstance_ID=?  AND C_AcctSchema_ID = ? ";
		Object[] params = { Integer.valueOf(MFG_WorkOrderResTxnLine_ID), Integer.valueOf(M_AttributeSetInstance_ID),
				Integer.valueOf(as.getC_AcctSchema_ID()) };
		int no = DB.executeUpdateEx(sql, params, trxName);
		if (no != 0)
			s_log.config("Deleted #" + no);
		MCostDetail cd = get(as.getCtx(), "MFG_WorkOrderResTxnLine_ID=? AND M_AttributeSetInstance_ID=?",
				MFG_WorkOrderResTxnLine_ID, M_AttributeSetInstance_ID, trxName);
		if (cd == null) {
			cd = new MCostDetail(as, AD_Org_ID, M_Product_ID, M_AttributeSetInstance_ID, M_CostElement_ID, Amt, Qty,
					Description, trxName);
			cd.setMFG_WorkOrderResTxnLine_ID(MFG_WorkOrderResTxnLine_ID);
			cd.setIsSOTrx(IsSOTrx);
		} else {
			cd.setDeltaAmt(cd.getAmt().subtract(Amt));
			cd.setDeltaQty(cd.getQty().subtract(Qty));
	...
	public static boolean createJobWork (MAcctSchema as, int AD_Org_ID, 
			int M_Product_ID, int M_AttributeSetInstance_ID,
			int M_InOutLine_ID, int M_CostElement_ID, 
			BigDecimal Amt, BigDecimal Qty,
			String Description, boolean IsSOTrx, String trxName)
		{
			...
			if (cd == null)		//	createNew
			{
				...
				cd.setM_InOutLine_ID(M_InOutLine_ID);
				cd.setIsSOTrx(IsSOTrx);
				cd.set_ValueNoCheck("IsJobWorkPlus", "Y");
			}
			else
			{
				// MZ Goodwill
