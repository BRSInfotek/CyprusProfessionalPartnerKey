...
public class MInOut extends X_M_InOut implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
        ...
        if (getC_Order_ID() != 0 && getM_RMA_ID() != 0 && !dt.getDocBaseType().equalsIgnoreCase("JWR") && !dt.getDocBaseType().equalsIgnoreCase("JWI"))
        {
            log.saveError("OrderOrRMA", "");
            return false;
        }

        // code update by Anshul
        if(!dt.getDocBaseType().equalsIgnoreCase("JWR") && !dt.getDocBaseType().equalsIgnoreCase("JWI"))
        {
        ...
        // code update by Anshul
        if(dt.getDocBaseType().equalsIgnoreCase("JWR"))
        {
        	setMovementType("J+");
        }
        if(dt.getDocBaseType().equalsIgnoreCase("JWI"))
        {
        	setMovementType("J-");
        }
...
//		}
     ...
     // Query to check if the document type is "Job Work Issue" or "Job Work Receipt" and if Job Work is enabled
        if(newRecord || is_ValueChanged(COLUMNNAME_C_DocType_ID))
        {
        	String sql = "SELECT IsJobWork FROM C_DocType WHERE C_DocType_ID=? AND DocBaseType IN ('JWI', 'JWR')"; 
        	String jobWorkFlag = DB.getSQLValueString(get_TrxName(), sql, getC_DocType_ID());
        	// If the document type is "Job Work Issue" or "Job Work Receipt" and Job Work is enabled, set the checkbox
        	boolean isJobWork = "Y".equals(jobWorkFlag);
        	set_Value("IsJobWork", isJobWork);
        }
		return true;
	...
	public String completeIt()
	{
		...
		MDocType dtDoc = new MDocType(getCtx(), getC_DocType_ID(), get_TrxName());
		MInOutLine[] InOutlinesr = getLines(false);
		for (MInOutLine line : InOutlinesr)
		{
			if("MMR".equalsIgnoreCase(dtDoc.getDocBaseType()) && dtDoc.get_ValueAsBoolean("IsQualityCheck"))
			{
				int count = DB.getSQLValue(null, "SELECT Count(*) FROM QC_MRQualityResult WHERE M_InOutLine_ID=" + line.getM_InOutLine_ID());					
				if(count <= 0)
				{
					return  m_processMsg = Msg.getMsg(getCtx(), "Cyprus_MRQualityCheck");
				}
			}
		}
		
		String docSubTypeSO = DB.getSQLValueString(get_TrxName(), "SELECT DocSubTypeSO FROM C_Order WHERE C_Order_ID=?", getC_Order_ID());
		...
		for (int lineIndex = 0; lineIndex < lines.length; lineIndex++)
		{
			...
			if (product != null
				&& product.isStocked() )
			{
				String docBaseType = DB.getSQLValueStringEx(get_TrxName(), "SELECT DocBaseType, GL_Category_ID FROM C_DocType WHERE C_DocType_ID=?", getC_DocType_ID());					
				MDocType dt = new MDocType(getCtx(), getC_DocType_ID(), get_TrxName());
				...
				if(this.get_ValueAsInt("C_JobOrder_ID") > 0)
				{
					int warehouse_ID = DB.getSQLValue(null, "SELECT M_Warehouse_ID FROM C_JobOrder WHERE C_JobOrder_ID=" + this.get_ValueAsInt("C_JobOrder_ID"));
					reservationAttributeSetInstance_ID = DB.getSQLValue(null, "SELECT M_AttributeSetInstance_ID FROM C_JobOrderLine WHERE C_JobOrderLine_ID=" + sLine.get_ValueAsInt("C_JobOrderLine_ID"));
					sameWarehouse = warehouse_ID==getM_Warehouse_ID();
				}
				//
...
//							get_TrxName()))
						if (!MStorage.addStorage(getCtx(), getM_Warehouse_ID(),
								sLine.getM_Locator_ID(),
								sLine.getM_Product_ID(),
								ma.getM_AttributeSetInstance_ID(), reservationAttributeSetInstance_ID,
								QtyMA,
						...
						if (!sameWarehouse) {
							...
							{
								int warehouse_ID = DB.getSQLValue(null, "SELECT M_Warehouse_ID FROM C_JobOrder WHERE C_JobOrder_ID=" + this.get_ValueAsInt("C_JobOrder_ID"));
								 wh = MWarehouse.get(getCtx(), warehouse_ID);
//									if (!MStorage.add(getCtx(), warehouse_ID,
//											wh.getDefaultLocator().getM_Locator_ID(),
//											sLine.getM_Product_ID(),
//											ma.getM_AttributeSetInstance_ID(), reservationAttributeSetInstance_ID,
//											Env.ZERO, reservedDiff, orderedDiff, get_TrxName()))
								 if (!MStorage.addStorage(getCtx(), oLine.getM_Warehouse_ID(),
											wh.getDefaultLocator().getM_Locator_ID(),
											sLine.getM_Product_ID(),
											ma.getM_AttributeSetInstance_ID(), reservationAttributeSetInstance_ID,
											Env.ZERO, reservedDiff, orderedDiff, get_TrxName(),docSubTypeSO))		
								 		{
							...
							{
								wh = MWarehouse.get(getCtx(), oLine.getM_Warehouse_ID());
...
//										Env.ZERO, reservedDiff, orderedDiff, get_TrxName()))
						...
						// updated by Anshul @26072024 to create cost detail in case of shipment
						if(QtyMA.signum() != 0)
						{	
							String err = createCostDetailShipment(sLine, ma.getM_AttributeSetInstance_ID() , QtyMA);
...
//				// code commented by Anshul @26072024 to create cost detial for return to vendor
//				else
//				{
//					String docBaseType = DB.getSQLValueStringEx(get_TrxName(), "SELECT DocBaseType, GL_Category_ID FROM C_DocType WHERE C_DocType_ID=?", getC_DocType_ID());					
//					MDocType dt = new MDocType(getCtx(), getC_DocType_ID(), get_TrxName());
//					if ((MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) || (MDocType.DOCBASETYPE_MaterialReceipt.equals(docBaseType))) && dt.isReturnTrx())/*&& isSOTrx()*/ 
//					{
//						BigDecimal qtyShip = sLine.getMovementQty();
//						if (sLine.getM_Product_ID() != 0)
//						{
//							if(MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) && dt.isReturnTrx())
//							{
//								qtyShip = qtyShip.negate();
//							}
//							String err = createCostDetailShipment(sLine, sLine.getM_AttributeSetInstance_ID() , qtyShip);
//							if (err != null && err.length() > 0) {
//								m_processMsg = err;
//								return DocAction.STATUS_Invalid;
//							}
...
//						Qty, reservedDiff, orderedDiff, get_TrxName()))
					if (!MStorage.addStorage(getCtx(), getM_Warehouse_ID(),
							...
							sLine.getM_Product_ID(),
							sLine.getM_AttributeSetInstance_ID(), reservationAttributeSetInstance_ID,
							Qty, reservedDiff, orderedDiff, get_TrxName(),docSubTypeSO))
					{
					...
					if (!sameWarehouse) {
						MWarehouse wh = null;
						if(get_ValueAsInt("C_JobOrder_ID") > 0)
						{
							int warehouse_ID = DB.getSQLValue(null, "SELECT M_Warehouse_ID FROM C_JobOrder WHERE C_JobOrder_ID=" + this.get_ValueAsInt("C_JobOrder_ID"));
							 wh = MWarehouse.get(getCtx(), warehouse_ID);
//								if (!MStorage.add(getCtx(), warehouse_ID,
...
//										sLine.getM_AttributeSetInstance_ID(), reservationAttributeSetInstance_ID,
//										Env.ZERO, reservedDiff, orderedDiff, get_TrxName()))
							 if (!MStorage.addStorage(getCtx(), warehouse_ID,
										wh.getDefaultLocator().getM_Locator_ID(),
										sLine.getM_Product_ID(),
										sLine.getM_AttributeSetInstance_ID(), reservationAttributeSetInstance_ID,
										Env.ZERO, reservedDiff, orderedDiff, get_TrxName(),docSubTypeSO))
...
//									Env.ZERO, QtySO.negate(), QtyPO.negate(), get_TrxName()))
								if (!MStorage.addStorage(getCtx(), oLine.getM_Warehouse_ID(),
										...
										sLine.getM_Product_ID(),
										sLine.getM_AttributeSetInstance_ID(), reservationAttributeSetInstance_ID,
										Env.ZERO, QtySO.negate(), QtyPO.negate(), get_TrxName(),docSubTypeSO))
					...
					for(int asn = 0; asn < acctschemas.length; asn++)
					{
						...
						if (((MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) || (MDocType.DOCBASETYPE_MaterialReceipt.equals(docBaseType))) && dt.isReturnTrx()) || docBaseType.equalsIgnoreCase("JWR") || docBaseType.equalsIgnoreCase("JWI") ||  (product.getCostingLevel(as).equalsIgnoreCase("B")))/*&& isSOTrx()*/ 
						{
							BigDecimal qtyShip = sLine.getMovementQty();
								if (sLine.getM_Product_ID() != 0)
								{
									if(MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) || docBaseType.equalsIgnoreCase("JWI"))
									{
										qtyShip = qtyShip.negate();
									}
									if(docBaseType.equalsIgnoreCase("JWR"))
									{
										BigDecimal cost = getJobOrderCost(sLine);
										MCostDetail.createJobWork(as, sLine.getAD_Org_ID(), sLine.getM_Product_ID(), sLine.getM_AttributeSetInstance_ID(), sLine.getM_InOutLine_ID(), 0, cost, qtyShip, sLine.getDescription(), false, get_TrxName());
										if(cost != null && qtyShip.signum() != 0)
										{
											MCostDetail cd = MCostDetail.get (as.getCtx(), "M_InOutLine_ID=?", 
													sLine.getM_InOutLine_ID(), sLine.getM_AttributeSetInstance_ID(), as.getC_AcctSchema_ID(), get_TrxName());
											 if(cd != null)
											 {
											     cd.setCurrentCostPrice(cost.abs());
												 cd.save();
											 }
											int updateIOL = DB.executeUpdateEx("UPDATE M_InOutLine SET TrxCost=" + cost.abs() + " WHERE M_InOutLine_ID=" + sLine.getM_InOutLine_ID(), get_TrxName());
											log.saveInfo("", "" + updateIOL);
									
											int updateTrx = DB.executeUpdateEx("UPDATE M_Transaction SET TrxCost=" + cost.abs() + " , IsJobWorkP ='Y' WHERE M_InOutLine_ID=" + sLine.getM_InOutLine_ID(), get_TrxName());
											log.saveInfo("", "" + updateTrx);
										}
									}
									else
									{
...
//						MAcctSchema as = acctschemas[asn];
//						if (((MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) || (MDocType.DOCBASETYPE_MaterialReceipt.equals(docBaseType))) && dt.isReturnTrx()) ||  (product.getCostingLevel(as).equalsIgnoreCase("B")))/*&& isSOTrx()*/ 
//						{
//							BigDecimal qtyShip = sLine.getMovementQty();
//								if (sLine.getM_Product_ID() != 0)
//								{
//									if(MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType))
//									{
//										qtyShip = qtyShip.negate();
//									}		
//									String err = createCostDetailShipment(sLine, sLine.getM_AttributeSetInstance_ID() , qtyShip);
...
//						}
            ...
            /**
			 *
			 // Commented below functionality to create Assets from Shipment as per discussion with Surya @20221118 by Mukesh
			//	Create Asset for SO
			if (product != null
				&& isSOTrx()
			...
			// code update by Anshul @05072024 to update cost and cost detail on shipment
//			
			String docBaseType = DB.getSQLValueStringEx(get_TrxName(), "SELECT DocBaseType, GL_Category_ID FROM C_DocType WHERE C_DocType_ID=?", getC_DocType_ID());
...
//			
			...
			// update by Anshul @25072024 to calculate other cost also on MR Complete
			if(MDocType.DOCBASETYPE_MaterialReceipt.equals(docBaseType) && !dt.isReturnTrx())
			{
				updateOtherCost(sLine, product);	
			}
...
//							MProduct product = new MProduct(getCtx(), line.getM_Product_ID(), get_TrxName());
	...
	public boolean voidIt()
	{
		...
		for (MInOutLine line : InOutlines)
		{
			if(UtilTransaction.isVoidBackDatedRecordTransaction(getMovementDate(),UtilTransaction.MATERIALRECEIPT,getAD_Client_ID(),getAD_Org_ID(),line.getM_Product_ID()))
			{
			...
			|| DOCSTATUS_NotApproved.equals(getDocStatus()) )
		{
			...
			for (int i = 0; i < lines.length; i++)
			{
				...
				if (old.signum() != 0)
				{
					...
					// below code update by Anshul @20210824 regarding current costing and current stock
	...
	public MProduct getProduct(MInOutLine line)
	{
	...
	public BigDecimal getCost(MInOutLine line,MAcctSchema as)
	{
		MInOutLineMA[] m_linesma = MInOutLineMA.get(getCtx(), line.getM_InOutLine_ID(), get_TrxName());
		BigDecimal cost = Env.ZERO;
		BigDecimal qty = Env.ZERO;
			for(int ma = 0; ma < m_linesma.length; ma++)
			{
				MInOutLineMA lineMA = m_linesma[ma];
	...
	// Update by Anshul @29072024 to create cost detail on Shipment complete
	private String createCostDetailShipment(MInOutLine line, int M_AttributeSetInstance_ID, BigDecimal qty)
	{
		...
		for(int asn = 0; asn < acctschemas.length; asn++)
		{
			...
			if (isReversal())
			{				
				String sql = "SELECT amt * -1 FROM M_CostDetail WHERE M_InOutLine_ID=?"; // negate costs				
			...
			else if((MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) && !dt.isSOTrx() && dt.isReturnTrx()))
			{
				BigDecimal POCost = DB.getSQLValueBD(get_TrxName(), "SELECT il.TrxCost FROM M_InOutLine il LEFT JOIN M_RMALine rl ON (il.M_InOutLine_ID=rl.M_InOutLine_ID) WHERE rl.M_RMALine_ID = ?" ,line.getM_RMALine_ID());
				if(POCost != null)
				{
					costs = POCost.multiply(qty);
				}
			}
			else if((MDocType.DOCBASETYPE_MaterialReceipt.equals(docBaseType) && dt.isSOTrx() && dt.isReturnTrx()))
			{
				BigDecimal POCost = DB.getSQLValueBD(get_TrxName(), "SELECT il.TrxCost FROM M_InOutLine il LEFT JOIN M_RMALine rl ON (il.M_InOutLine_ID=rl.M_InOutLine_ID) WHERE rl.M_RMALine_ID = ?" ,line.getM_RMALine_ID());
				if(POCost != null)
				{
			...
			{				
				if(MProductCategoryAcct.COSTINGMETHOD_Lifo.equals(costingMethod) || MProductCategoryAcct.COSTINGMETHOD_Fifo.equals(costingMethod))
				{
					...
					costs = getCost(line,as).multiply(qty); 
					String sql = "SELECT " +
							"COALESCE(SUM(c.CurrentCostPrice),0) " + 
							"FROM adempiere.M_Cost c " + 
							"LEFT OUTER JOIN adempiere.M_CostElement ce ON (c.M_CostElement_ID=ce.M_CostElement_ID) " + 
					...
					additionalCost = DB.getSQLValueBD(get_TrxName(), sql, product.getAD_Client_ID(), AD_Org_ID, line.getM_Product_ID(), as.getM_CostType_ID(), as.getC_AcctSchema_ID());
			...
			// Set Total Amount and Total Quantity from Shipment

			MCostDetail.createShipment(as, line.getAD_Org_ID(), 
					line.getM_Product_ID(), M_AttributeSetInstance_ID,
					line.getM_InOutLine_ID(), 0,
				costs, qty,
				line.getDescription(), true, get_TrxName());
			

		    if(MDocType.DOCBASETYPE_MaterialDelivery.equals(docBaseType) && !dt.isReturnTrx() || docBaseType.equalsIgnoreCase("JWI")
			...
			{						
				if(costs != null && qty.signum() != 0)
				{
					MCostDetail cd = MCostDetail.get (as.getCtx(), "M_InOutLine_ID=?", 
							line.getM_InOutLine_ID(), M_AttributeSetInstance_ID, as.getC_AcctSchema_ID(), get_TrxName());
					 trxCost = costs.divide(qty, as.getCostingPrecision(), BigDecimal.ROUND_HALF_UP);
					 if(cd != null)
					 {
		    ...
		    if(trxCost.signum() != 0)
		    {
				int updateInOut = DB.executeUpdateEx("UPDATE M_InOutLine SET TrxCost=" + trxCost.abs() + " WHERE M_InOutLine_ID=" + line.getM_InOutLine_ID(), get_TrxName());
				log.saveInfo("", "" + updateInOut);
		
				String updateTrxSql = "";
				if(docBaseType.equalsIgnoreCase("JWI"))
				{
					updateTrxSql = "UPDATE M_Transaction SET TrxCost=" + trxCost.abs() + " , IsJobWorkM = 'Y' WHERE M_InOutLine_ID=" + line.getM_InOutLine_ID();
				}
				else
				{
					updateTrxSql = "UPDATE M_Transaction SET TrxCost=" + trxCost.abs() + " WHERE M_InOutLine_ID=" + line.getM_InOutLine_ID();
				}
		
				int updateTrx = DB.executeUpdateEx(updateTrxSql, get_TrxName());
				log.saveInfo("", "" + updateTrx);
		    }
	...
	//updated by Anshul @26072024 to update other cost also on MR Complete
	public void updateOtherCost(MInOutLine sLine, MProduct product)
	{
		MCostElement[] costElement = MCostElement.getElementsWithNoCostingMethod(getCtx(), get_TrxName(), Env.getAD_Org_ID(getCtx()));
			//Costing Level
		MAcctSchema[] acctschemas = MAcctSchema.getClientAcctSchema(getCtx(), getAD_Client_ID());	
			for(int cel=0; cel<costElement.length; cel++)
			{
				for(int asn = 0; asn < acctschemas.length; asn++)
				{
					MAcctSchema as = acctschemas[asn];
					MCostElement costele = costElement[cel];
					MCost landedCost = MCost.get(product, 0, as, sLine.getAD_Org_ID(), costele.getM_CostElement_ID(),get_TrxName());
					...
					if (MAcctSchema.COSTINGLEVEL_Organization.equals(CostingLevel))
						...
						sql += " AND M_AttributeSetInstance_ID=" + sLine.getM_AttributeSetInstance_ID();	
					//
					
//					// code commented by Anshul @23072024 
					BigDecimal qtyOnhand = DB.getSQLValueBD(get_TrxName(), sql);						
					
					if(landedCost != null && sLine.getMovementQty().signum() > 0 && qtyOnhand.signum() != 0)
					{
						BigDecimal oldCost = landedCost.getCurrentCostPrice().multiply(qtyOnhand);
						BigDecimal newCost = oldCost.divide(qtyOnhand, as.getCostingPrecision(), BigDecimal.ROUND_HALF_UP);
						landedCost.setCurrentCostPrice(newCost);
						landedCost.setCurrentQty(qtyOnhand);
						if(landedCost.getCumulatedQty().signum() != 0)
						{
							landedCost.setCumulatedQty(landedCost.getCumulatedQty().add(sLine.getMovementQty()));
						}
						else
						{
							landedCost.setCumulatedQty(qtyOnhand);
						}
						landedCost.save();
					}	
				}
	...
	public BigDecimal getJobOrderCost(MInOutLine line)
	{
		BigDecimal cost = BigDecimal.valueOf(0);
		int docTypeID = DB.getSQLValue(null, "SELECT C_DocType_ID FROM C_DocType WHERE DocBaseType='JWI'");
		String sql = "SELECT M_InOut_ID FROM M_InOut WHERE C_JobOrder_ID=" + this.get_ValueAsInt("C_JobOrder_ID") + " AND C_DocType_ID=" + docTypeID;
		MJobOrder jo = new MJobOrder(getCtx(), this.get_ValueAsInt("C_JobOrder_ID"), null); 
		MJobOrderLine[] jol = jo.getJobOrderLines(false);
		int jobWork_ID = DB.getSQLValue(null, sql);
		MInOut io = new MInOut(getCtx(), jobWork_ID, null);
		MInOutLine[] lines = io.getLines(false);
		if(jo.get_Value("JobOrderType").equals("IO"))
		{
			if(lines.length > 0)
			{
				for(int j = 0; j < lines.length; j++)
				{
					cost = cost.add(lines[j].getTrxCost().multiply(lines[j].getMovementQty()));				
				}
				
				for(int i = 0; i < jol.length; i++)
				{
					if(jol[i].getM_Product_ID() > 0)
					{
						if(jol[i].getQtyConverted() != BigDecimal.valueOf(0))
						{
							cost = cost.divide(jol[i].getQtyConverted(), 3, RoundingMode.HALF_UP);
						}						
					}
				}
			}
		...
		else
		{
			for(int i = 0; i < jol.length; i++)
			{
				if(jol[i].getM_Product_ID() > 0)
				{
					cost = jol[i].get_ValueAsBigDecimal("CurrentCostPrice");
				}
			}
		}
