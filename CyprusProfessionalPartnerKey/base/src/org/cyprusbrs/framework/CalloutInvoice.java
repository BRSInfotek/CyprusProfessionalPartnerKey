...
public class CalloutInvoice extends CalloutEngine
{
	...
	//	Discount = (BigDecimal)mTab.getValue("Discount");
		...
		// 07072025 updated by Anshul to set tax base amount
		mTab.setValue("TaxBaseAmt", LineNetAmt);

		
		// Updated by Mukesh @20250529 Regarding CRM 
		int C_Invoice_ID = Env.getContextAsInt(ctx, WindowNo, "C_Invoice_ID");
		if(C_Invoice_ID>0 && M_Product_ID>0) // for Invoice
		{
			if(!mField.getColumnName().equals("C_Tax_ID"))
			{
				BigDecimal lineNetAmt=(BigDecimal)mTab.getValue("LineNetAmt");
				Integer taxId=UtilTax.getTaxIDBasedOnState(MInvoice.Table_ID,C_Invoice_ID,M_Product_ID,lineNetAmt,PriceEntered);
	...
	// set Difference Amt
		// org.cyprusbrs.model.CalloutInvoice.calculatePriceDifference
		public String calculatePriceDifference(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) 
		{
	        if (value == null) return "";
	        
	      //  Integer jobOrderLineId = (Integer) value;
	        BigDecimal jobOrderLinePrice=null;
	        BigDecimal inOutLinePrice = Env.ZERO;
	        Integer jobOrderLineId = (Integer)mTab.getValue("C_JobOrderLine_ID");
	        Integer inOutLineID = (Integer)mTab.getValue("M_InOutLine_ID");;
	        if (jobOrderLineId!=null && jobOrderLineId>0) 
	        {
	        	 // Fetch price from C_JobOrderLine
		        jobOrderLinePrice = DB.getSQLValueBD(null,
		                "SELECT PriceEntered FROM C_JobOrderLine WHERE C_JobOrderLine_ID = ?",
		                jobOrderLineId);
		        if (jobOrderLinePrice == null)
		            jobOrderLinePrice = Env.ZERO;

		        // Get Invoice Line Price
		        BigDecimal invoiceLinePrice = (BigDecimal) mTab.getValue("PriceEntered");
		        if (invoiceLinePrice == null)
		            invoiceLinePrice = Env.ZERO;

		        // Calculate difference
		        BigDecimal difference = invoiceLinePrice.subtract(jobOrderLinePrice);

		        // Set the difference to DifferenceAmt field
		        mTab.setValue("DifferenceAmt", difference);
	        }
	        else if(inOutLineID!=null && inOutLineID > 0)
	        {
	        	inOutLinePrice = DB.getSQLValueBD(null,
		                "SELECT TrxCost FROM M_InOutLine WHERE M_InOutLine_ID = ?",
		                inOutLineID);
	        	BigDecimal invoiceLinePrice = (BigDecimal) mTab.getValue("PriceEntered");
	        	BigDecimal invoiceqty = (BigDecimal) mTab.getValue("QtyInvoiced");
	        	
		        if (invoiceLinePrice == null)
		            invoiceLinePrice = Env.ZERO;

		        // Calculate difference
		        BigDecimal difference = invoiceLinePrice.subtract(inOutLinePrice).multiply(invoiceqty);
		...
		// path : org.cyprusbrs.model.CalloutInvoice.getWithholdingTaxAmt
		public String getWithholdingTaxAmt (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
		{
...
//			    "SELECT DocBaseType FROM C_DocType dt JOIN C_Invoice inv " +
//			    "ON (inv.C_DocTypeTarget_ID = dt.C_DocType_ID) " +
//			    "WHERE C_Invoice_ID=?", 
//			    mTab.getValue("C_Invoice_ID"));
			...
			if("API".equalsIgnoreCase(docBaseType) || "APC".equalsIgnoreCase(docBaseType)) {
				calculationOn = DB.getSQLValueString(null, 
				        ...
				        new Object[] { Integer.valueOf(inv.getC_DocTypeTarget_ID()) });
			...
			{
				priceEntered = (BigDecimal)mTab.getValue("LineNetAmt");
			}
			else if ("T".equalsIgnoreCase(calculationOn))
			{
				priceEntered = (BigDecimal)mTab.getValue("TaxAmt");
			}
			...
			return "";
		}	//	getWithholdingTaxAmt
		
		// path : org.cyprusbrs.model.CalloutInvoice.getTCSAmt
				public String getTCSAmt (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
				{
					Integer productId = (Integer)mTab.getValue("M_Product_ID");
					int m_product_id = productId != null ? productId.intValue() : 0;
					int C_Invoice_ID = ((Integer)mTab.getValue ("C_Invoice_ID"))
					...
					if(m_product_id > 0) 
					{
						...
						BigDecimal priceEntered = (BigDecimal)mTab.getValue("LineTotalAmt");
						priceEntered = priceEntered.multiply (rate).setScale (
								2, BigDecimal.ROUND_HALF_UP);
						if(priceEntered != null && percent != null && percent.compareTo(Env.ZERO) > 0) {
					...
					{
						mTab.setValue("TCSAmt", Env.ZERO);
					}

					return "";
				}	//	getTCSAmt
	
		//path: org.cyprusbrs.model.CalloutInvoice.setTaxBasedOnState
		public String setTaxBasedOnState(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) 
		{
			Integer taxId=null;
			// Exit early if Product is null
			if (value == null)
				return "";
			...
			taxId=UtilTax.getTaxIDBasedOnState(MInvoice.Table_ID,C_Invoice_ID,M_Product_ID,lineNetAmt,price);
		...
		//path: org.cyprusbrs.model.CalloutInvoice.updateTaxBasedOnCharge
