...
public class MMeasure extends X_PA_Measure
{
	...
	public ArrayList<GraphColumn> getGraphColumnList(MGoal goal)
	{
		...
		else if (X_PA_Measure.MEASURETYPE_Achievements.equals(getMeasureType()))
		{
			if (X_PA_Measure.MEASUREDATATYPE_StatusQtyAmount.equals(getMeasureDataType()))
			{
		...
		for (MGoal goal:goals)
		{
			...
			while (st.hasMoreTokens())      //  for each class
			{
				...
				if (cmd.toLowerCase().startsWith(MRule.SCRIPT_PREFIX)) {
					...
					if ( !  (rule.getEventType().equals(X_AD_Rule.EVENTTYPE_MeasureForPerformanceAnalysis) 
