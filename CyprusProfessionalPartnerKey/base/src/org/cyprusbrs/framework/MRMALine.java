...
public class MRMALine extends X_M_RMALine
{
    ...
    protected boolean beforeSave(boolean newRecord)
    {
    	...
    	"SELECT dt.DocBaseType FROM M_RMA rma INNER JOIN C_DocType dt ON (dt.C_DocType_ID = rma.C_DocType_ID) WHERE rma.M_RMA_ID = ?", getM_RMA_ID());
        ...
        if (m_ioLine != null)
        {
        	...
        	if(X_C_DocType.DOCBASETYPE_PurchaseOrder.equals(docBaseType) && !checkOnHandQty())
        	{
				return false;
        	}
            if (newRecord || is_ValueChanged(COLUMNNAME_M_InOutLine_ID))
            {
                ...
                int lineIds[] = PO.getAllIDs(I_M_RMALine.Table_Name, whereClause, this.get_TrxName());
    ...
    /**
     * Code updated by Anshul @20240808
     * @return
     */
    public boolean checkOnHandQty() {
    	MStorage storage = MStorage.get(getCtx(), getM_Locator_ID(),
			...
			if (storage != null)
			{
				...
				if (getQty().compareTo(qty) > 0)
				{
					log.warning("Qty - Stock=" + qty + ", Movement=" + getQty());
