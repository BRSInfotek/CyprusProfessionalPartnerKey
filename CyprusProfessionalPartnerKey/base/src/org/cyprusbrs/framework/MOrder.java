	...
	for (int i = 0; i < taxes.length; i++)
	{
		...
		if (tax.isSummary())
		{
			...
			for (int j = 0; j < cTaxes.length; j++)
			{
				...
				{
					MOrderTax newOTaxRC = new MOrderTax(getCtx(), 0, get_TrxName());
					newOTaxRC.setClientOrg(this);
					newOTaxRC.setC_Order_ID(getC_Order_ID());
					newOTaxRC.setC_Tax_ID(cTax.getC_Tax_ID());
					newOTaxRC.setPrecision(getPrecision());
					newOTaxRC.setIsTaxIncluded(isTaxIncluded());
					newOTaxRC.setTaxBaseAmt(oTax.getTaxBaseAmt());
					newOTaxRC.setTaxAmt(taxAmt.negate());
					newOTaxRC.set_Value("IsReverseCharge", "Y");
					newOTaxRC.saveEx(get_TrxName());
					if (!isTaxIncluded())
...
//				return false;
	...
	public String completeIt()
	{
		...
		if (MDocType.DOCSUBTYPESO_Proposal.equals(DocSubTypeSO)
			...
			|| MDocType.DOCSUBTYPESO_Manual.equals(DocSubTypeSO)) 
		{
			//	Binding
		...
		// update by Anshul @21072024 to Set Transaction Cost on orderline
		
		MOrderLine[] lines = getLines();
		for (int lineIndex = 0; lineIndex < lines.length; lineIndex++)
		{
			MOrderLine oLine = lines[lineIndex];
			BigDecimal Cost = oLine.getPriceActual();
			...
			if(Cost.signum() != 0)
			{
				int updateOrderLine = DB.executeUpdateEx("UPDATE C_OrderLine SET TrxCost=" + Cost.abs() + " WHERE C_OrderLine_ID=" + oLine.getC_OrderLine_ID(), get_TrxName());
				log.saveInfo("", "" + updateOrderLine);
			}
//			oLine.setTrxCost(Cost);
//			if(!oLine.save())
//				return "Transaction Cost not updated.";
		}
	...
	public MInOut getOpenInOut (int C_DocType_ID, int M_Warehouse_ID, int C_BPartner_ID, int C_BPartner_Location_ID)
	{
		...
		try
		{
			...
			pstmt.setInt(5, C_DocType_ID);
			 rs = pstmt.executeQuery();
			if (rs.next())
				inout = new MInOut(getCtx(), rs.getInt(1), get_TrxName());
			rs.close();
			pstmt.close();
			pstmt = null;
		}
