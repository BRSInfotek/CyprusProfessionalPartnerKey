...
import org.cyprusbrs.model.Tax;
...
//				setM_Product_ID(0);
		...
		set_Value("TaxBaseAmt", getLineNetAmt());
		
		if(newRecord) 
		{
			int count = DB.getSQLValue(null, "SELECT Count(*) FROM C_Invoice WHERE C_Invoice_ID=" + getC_Invoice_ID());
			if(count > 0)
			{
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		{
			String isReverseCharge = DB.getSQLValueString(null, "SELECT IsReverseCharge FROM C_Tax Where C_Tax_ID=?", getC_Tax_ID());
			//	Recalculate Tax for old Tax
			if (!updateInvoiceTax(true))
				return false;
			if("Y".equalsIgnoreCase(isReverseCharge))
			{
				if (!updateInvoiceTaxForReverseCharge(true))
					return false;
			}
		}
	...
	private boolean updateHeaderTax()
	{
		...
		String isReverseCharge = "N";
		if(getC_Tax_ID() > 0)
		{
			 isReverseCharge = DB.getSQLValueString(null, "SELECT IsReverseCharge FROM C_Tax Where C_Tax_ID=?", getC_Tax_ID());
		}
		...
		if("Y".equalsIgnoreCase(isReverseCharge))
		{
			if (!updateInvoiceTaxForReverseCharge(false))
				return false;
		}

		// commented by anshul @22-04-2025 (changes regarding TDS)
...
//		Update Invoice Header
			...
			{
				sql = "UPDATE C_Invoice i "
						+ " SET GrandTotalBeforeWithholding=TotalLines "
						+ "WHERE C_Invoice_ID=?";
			}			
			else
			{
			...
			// update Grand Total after withholding
						String sqlUpdate = "UPDATE C_Invoice i"
							+ " SET GrandTotal="
								+ "COALESCE((SELECT COALESCE(GrandTotalBeforeWithholding,0)-SUM(COALESCE(WithholdingTaxAmt,0)) + SUM(COALESCE(TCSAmt,0)) FROM C_InvoiceLine il WHERE i.C_Invoice_ID=il.C_Invoice_ID),0) "
							+ " , TotalWithholdingAmt="
							+ "(SELECT COALESCE(SUM(WithholdingTaxAmt),0) FROM C_InvoiceLine il WHERE i.C_Invoice_ID=il.C_Invoice_ID) "
							+ "WHERE C_Invoice_ID=?";
						int noUpdate = DB.executeUpdateEx(sqlUpdate, new Object[]{getC_Invoice_ID()}, get_TrxName());
						if (noUpdate != 1)
							log.warning("(1) #" + noUpdate);
						
			// update Total TCS
			String sqlUpdateTCS = "UPDATE C_Invoice i"
				+ " SET TCSTotalAmt="
				+ "(SELECT COALESCE(SUM(TCSAmt),0) FROM C_InvoiceLine il WHERE i.C_Invoice_ID=il.C_Invoice_ID) "
				+ "WHERE C_Invoice_ID=?";
			int noUpdateTCS = DB.executeUpdateEx(sqlUpdateTCS, new Object[]{getC_Invoice_ID()}, get_TrxName());
			if (noUpdateTCS != 1)
				log.warning("(1) #" + noUpdateTCS);
	...
	public String allocateLandedCosts()
	{
		...
		if (lcs.length == 1)
		{
			...
			if (lc.getM_InOut_ID() != 0 && lc.getM_InOutLine_ID() == 0)
			{
				...
				{
					MInOutLine iol = (MInOutLine)list.get(i);
					MProduct product = new MProduct(getCtx(), iol.getM_Product_ID(), null);
					String sql1 = "SELECT COALESCE(SUM(QtyOnHand),0) FROM M_Storage s WHERE s.M_Product_ID=" + iol.getM_Product_ID() + " AND AD_Client_ID=" + product.getAD_Client_ID() + " AND AD_Org_ID=" + product.getAD_Org_ID();	
					BigDecimal onHandQty = DB.getSQLValueBD(null, sql1);
					MLandedCostAllocation lca = new MLandedCostAllocation (this, lc.getM_CostElement_ID());
					lca.set_ValueNoCheck("M_InOutLine_ID", iol.getM_InOutLine_ID());
					lca.set_ValueNoCheck("QtyOnHand", onHandQty);
	...
	// create by anshul @20250214
		public void createTransaction(MInvoice invoice, String MovementType)
		{
			MLandedCostAllocation[] allocations = MLandedCostAllocation.getOfInvoiceLine(
					getCtx(), getC_InvoiceLine_ID(), get_TrxName());
			int InOutLineID = 0;
			...
			for(int asn = 0; asn < acctschemas.length; asn++)
			{
				...
				{
					MLandedCostAllocation allocation = allocations[i];
					InOutLineID = allocations[0].get_ValueAsInt("M_InOutLine_ID");
					qty = qty.add(allocation.getQty());
					if(qty != BigDecimal.valueOf(0))
					{
						amount = amount.add(allocation.getAmt().multiply(allocation.getQty()));
					}
				}
				
				if(qty != BigDecimal.valueOf(0))
				{
					cost = amount.divide(qty, 3, RoundingMode.HALF_UP);
				}
					
					if (invoice.getC_Currency_ID() != as.getC_Currency_ID())
						cost = MConversionRate.convert(getCtx(), cost, 
								invoice.getC_Currency_ID(), as.getC_Currency_ID(),
								invoice.getDateAcct(), invoice.getC_ConversionType_ID(), 
							getAD_Client_ID(), getAD_Org_ID());
					if (cost.scale() > as.getCostingPrecision())
				...
				if(InOutLineID > 0)
				{
					MInOutLine iol = new MInOutLine(getCtx(), InOutLineID, get_TrxName());
					MTransaction mtrx = null;
					mtrx = new MTransaction (getCtx(), getAD_Org_ID(),
							MovementType, iol.getM_Locator_ID(),
							iol.getM_Product_ID(), iol.getM_AttributeSetInstance_ID(),
							getQtyEntered(), invoice.getDateInvoiced(), get_TrxName());
					mtrx.set_ValueNoCheck("C_InvoiceLine_ID", getC_InvoiceLine_ID());
					mtrx.set_ValueNoCheck("LandedCost", cost);
					mtrx.save();
				}	
			}
		}
		
		private boolean updateInvoiceTaxForReverseCharge(boolean oldTax) {
			String trxName = Trx.createTrxName("taxInvoice");
			Trx trx = Trx.get(trxName, true);
			MInvoiceTax tax = MInvoiceTax.get (this, getPrecision(), oldTax, get_TrxName());
			MInvoiceTax taxNew = new MInvoiceTax(getCtx(), 0, trxName);
			if (tax != null) {
				taxNew.set_TrxName(trxName);
				taxNew.setClientOrg(this);
				taxNew.setC_Invoice_ID(getC_Invoice_ID());
				taxNew.setC_Tax_ID(getC_Tax_ID());
				taxNew.set_Value("IsReverseCharge","Y");
				taxNew.setPrecision(getPrecision());
				taxNew.setIsTaxIncluded(isTaxIncluded());
				taxNew.setTaxBaseAmt(tax.getTaxBaseAmt());
				taxNew.setTaxAmt(tax.getTaxAmt().negate());
//				if (!taxNew.calculateTaxFromLinesForReverseCharge(getC_Invoice_ID(), getC_Tax_ID()))
//					return false;
			
				// red1 - solving BUGS #[ 1701331 ] , #[ 1786103 ]
				if (taxNew.getTaxAmt().signum() != 0) {
					if (!taxNew.save(trxName))
					{
						trx.rollback();
					    trx.close();
						return false;
					...
					if (!taxNew.is_new() && !taxNew.delete(false,trxName))
						return false;
				}
			}
			return true;
		...
		private boolean updateWithholdingTax() { 
			MInvoice inv = new MInvoice(getCtx(), getC_Invoice_ID(), null);
			int count = DB.getSQLValue(null, "SELECT Count(*) FROM C_Payment WHERE IsPrepayment='Y' AND IsAllocated='N' AND DocStatus IN ('CO','CL') AND C_Order_ID=" + inv.getC_Order_ID());
			if(count > 0)
			{
				return true;
			}
			...
			String docBaseType = DB.getSQLValueString(null, "SELECT DocBaseType FROM C_DocType WHERE C_DocType_ID=?",new Object[] { Integer.valueOf(inv.getC_DocTypeTarget_ID()) });
			if(getM_Product_ID() > 0) {
			    MProduct mp = MProduct.get(getCtx(), getM_Product_ID());
			    C_WithholdingCategory_ID = mp.get_ValueAsInt("C_WithholdingCategory_ID");
			} else if(getC_Charge_ID() > 0) {
			    MCharge ch = new MCharge(getCtx(), getC_Charge_ID(), null);
			...
			int C_Currency_ID = inv.getC_Currency_ID();
			int C_ConversionType_ID = inv.getC_ConversionType_ID();
			Timestamp ConvDate = inv.getDateInvoiced();
			MAcctSchema[] acctschemas = MAcctSchema.getClientAcctSchema(getCtx(), getAD_Client_ID());
			for(int asn = 0; asn < acctschemas.length; asn++)
			{
				MAcctSchema as = acctschemas[asn];
				...
				{						
					...
					if (rate == null)
					{
						log.saveError("Error", "Currency Conversion Not Available - " + as.getName()); 
						return false;
					}
				}
			}
			if("API".equalsIgnoreCase(docBaseType) || "APC".equalsIgnoreCase(docBaseType)) {
				calculationOn = DB.getSQLValueString(null, 
				        "SELECT InvCalculationOn FROM C_Withholding WHERE trxtype='P' AND IsApplicableonInv = 'Y' AND C_WithholdingCategory_ID=?", 
				        new Object[] { Integer.valueOf(C_WithholdingCategory_ID) });
			    percent = DB.getSQLValueBD(null, 
			        "SELECT Percent FROM C_Withholding WHERE trxtype='P' AND IsApplicableonInv = 'Y' AND C_WithholdingCategory_ID=?", 
			        C_WithholdingCategory_ID);
			    thresoldMin = DB.getSQLValueBD(null, 
				        "SELECT Thresholdmin FROM C_Withholding WHERE trxtype='P' AND IsApplicableonInv = 'Y' AND C_WithholdingCategory_ID=?", 
				        C_WithholdingCategory_ID);
			    thresoldMax = DB.getSQLValueBD(null, 
				        "SELECT ThresholdMax FROM C_Withholding WHERE trxtype='P' AND IsApplicableonInv = 'Y' AND C_WithholdingCategory_ID=?", 
				        C_WithholdingCategory_ID);
			} else if("ARI".equalsIgnoreCase(docBaseType) || "ARC".equalsIgnoreCase(docBaseType)) {
				calculationOn = DB.getSQLValueString(null, 
				        "SELECT InvCalculationOn FROM C_Withholding WHERE trxtype='S' AND IsApplicableonInv = 'Y' AND C_WithholdingCategory_ID=?", 
				        new Object[] { Integer.valueOf(C_WithholdingCategory_ID) });
			    percent = DB.getSQLValueBD(null, 
			        "SELECT Percent FROM C_Withholding WHERE trxtype='S' AND IsApplicableonInv = 'Y' AND C_WithholdingCategory_ID=?", 
			        C_WithholdingCategory_ID);
			    thresoldMin = DB.getSQLValueBD(null, 
				        "SELECT Thresholdmin FROM C_Withholding WHERE trxtype='S' AND IsApplicableonInv = 'Y' AND C_WithholdingCategory_ID=?", 
				        C_WithholdingCategory_ID);
			    thresoldMax = DB.getSQLValueBD(null, 
				        "SELECT ThresholdMax FROM C_Withholding WHERE trxtype='S' AND IsApplicableonInv = 'Y' AND C_WithholdingCategory_ID=?", 
				        C_WithholdingCategory_ID);
			}

			if("L".equalsIgnoreCase(calculationOn))
			{
				priceEntered = getLineNetAmt();
			}
			else if ("T".equalsIgnoreCase(calculationOn))
			{
				priceEntered = getTaxAmt();
			}
			if(thresoldMin!=null && thresoldMin.compareTo(Env.ZERO) > 0)
			{
				if(priceEntered.compareTo(thresoldMin) > 0)
				{
					priceEntered = priceEntered.multiply (rate).setScale (
							2, BigDecimal.ROUND_HALF_UP);
					 withholdingamt = Env.ZERO;
					if(priceEntered != null && percent != null && percent.compareTo(Env.ZERO) > 0) {
					     withholdingamt = priceEntered.multiply(percent);
					    if(withholdingamt.compareTo(Env.ZERO) > 0) {
					        withholdingamt = withholdingamt.divide(Env.ONEHUNDRED, 2, RoundingMode.HALF_UP);
					    }
					}
				}
				    set_Value("WithholdingTaxAmt", withholdingamt);
			}
			else if(thresoldMax!=null && thresoldMax.compareTo(Env.ZERO) > 0)
			{
				if(thresoldMax.compareTo(priceEntered) > 0)
				{
					priceEntered = priceEntered.multiply (rate).setScale (
							2, BigDecimal.ROUND_HALF_UP);
					 withholdingamt = Env.ZERO;
		...
		public boolean getTCSAmt()
		{
			...
			if(getM_Product_ID() > 0) 
			{
				...
				int tcs_ID = 0;
				BigDecimal percent = Env.ZERO;
				BigDecimal tcsAmt = Env.ZERO;					
			    MProduct mp = MProduct.get(getCtx(), getM_Product_ID());
			    tcs_ID = mp.get_ValueAsInt("M_TaxCollectedAtSource_ID");
			    percent = DB.getSQLValueBD(null, 
				        "SELECT Percent FROM M_TaxCollectedAtSource WHERE M_TaxCollectedAtSource_ID=?", 
				        tcs_ID);

				BigDecimal priceEntered = getLineTotalAmt();
				priceEntered = priceEntered.multiply (rate).setScale (
						2, BigDecimal.ROUND_HALF_UP);
				if(priceEntered != null && percent != null && percent.compareTo(Env.ZERO) > 0) {
					tcsAmt = priceEntered.multiply(percent);
				    if(tcsAmt.compareTo(Env.ZERO) > 0) {
				    	tcsAmt = tcsAmt.divide(Env.ONEHUNDRED, 2, RoundingMode.HALF_UP);
				    }
				    set_Value("TCSAmt", tcsAmt);
				}
			}
			return true;
		}
