...
public class MWebProject extends X_CM_WebProject
{
	...
	public static MWebProject get (Properties ctx, int CM_WebProject_ID)
	{
		...
		MWebProject retValue = s_cache.get (key);
		if (retValue != null)
			return retValue;
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (newRecord)
		{
			MTree_Base tree = new MTree_Base (getCtx(), 
				getName()+X_AD_Tree.TREETYPE_CMContainer, X_AD_Tree.TREETYPE_CMContainer, get_TrxName());
			if (!tree.save())
				return false;
			setAD_TreeCMC_ID(tree.getAD_Tree_ID());
			//
			tree = new MTree_Base (getCtx(), 
				getName()+X_AD_Tree.TREETYPE_CMContainerStage, X_AD_Tree.TREETYPE_CMContainerStage, get_TrxName());
			if (!tree.save())
				return false;
			setAD_TreeCMS_ID(tree.getAD_Tree_ID());
			//
			tree = new MTree_Base (getCtx(), 
				getName()+X_AD_Tree.TREETYPE_CMTemplate, X_AD_Tree.TREETYPE_CMTemplate, get_TrxName());
			if (!tree.save())
				return false;
			setAD_TreeCMT_ID(tree.getAD_Tree_ID());
			//
			tree = new MTree_Base (getCtx(), 
				getName()+X_AD_Tree.TREETYPE_CMMedia, X_AD_Tree.TREETYPE_CMMedia, get_TrxName());
			if (!tree.save())
				return false;
