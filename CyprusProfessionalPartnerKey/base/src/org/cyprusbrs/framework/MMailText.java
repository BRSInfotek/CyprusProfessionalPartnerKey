...
//			return "";
		...
		while (i != -1)
		{
			...
			if (token.startsWith("SELECT ")) {
		        value = DB.getSQLValueString(null, token);
		    }
...
//	}	//	translate
	...
	private String parseVariable(String variable, PO po) {
	    ...
	    // Handle context variables if no PO
	    if (po == null) {
	        String ctxValue = Env.getContext(po.getCtx(), variable);
	        return !Util.isEmpty(ctxValue) ? ctxValue : "";
	    }

	    // Try to resolve as PO attribute
	    try {
	        Object value = po.get_Value(variable);
	        if (value != null) {
	            return ReferenceResolver.resolveReference(po, variable, value);
	        }
	    } catch (Exception e) {
	        // Fall through
	    }

	    // Return original token if unresolved
	    return "@" + variable + "@";
	}
