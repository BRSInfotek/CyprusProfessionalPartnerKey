...
//			|| COSTELEMENTTYPE_Overhead.equals(getCostElementType())
	...
	public static int getMfgCostElement(int AD_Client_ID) {
	    ...
	    try {
	      ...
	      cPreparedStatement.setInt(1, AD_Client_ID);
	...
	//updated by Anshul @25072024 to get Costing element where Costing method is null
	public static MCostElement[] getElementsWithNoCostingMethod (Properties ctx, String trxName, int AD_Org_ID)
	{
		final String whereClause = "AD_Client_ID = ? AND (AD_Org_ID=0 OR AD_Org_ID=?) AND CostingMethod is null";
		List<MCostElement> list = new Query(ctx, Table_Name, whereClause, trxName)
					.setParameters(Env.getAD_Client_ID(ctx),AD_Org_ID)
					.list();
		MCostElement[] retValue = new MCostElement[list.size()];
