...
import org.cyprus.mrp.model.MMRPForecastLine;
import org.cyprus.mrp.model.X_I_Forecast;
import org.cyprusbrs.framework.MPeriod;
//import org.cyprusbrs.model.X_I_Forecast;
...
//      throw new Exception(CLogger.retrieveError().getName()); 
    ...
    if (this.m_deleteOldImported) {
      sql = (new StringBuffer("DELETE FROM I_Forecast WHERE I_IsImported='Y'")).append(clientCheck);
    ...
    sql = (new StringBuffer("UPDATE I_Forecast SET AD_Client_ID = COALESCE (AD_Client_ID,")).append(this.m_AD_Client_ID).append("), AD_Org_ID = COALESCE (AD_Org_ID,").append(this.m_AD_Org_ID).append("), IsActive = COALESCE (IsActive, 'Y'), Created = COALESCE (Created, SysDate), CreatedBy = COALESCE (CreatedBy, 0), Updated = COALESCE (Updated, SysDate), UpdatedBy = COALESCE (UpdatedBy, 0), I_ErrorMsg = NULL, I_IsImported = 'N' WHERE I_IsImported<>'Y' OR I_IsImported IS NULL");
   // no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    
    this.log.info("Reset=" + no);
    String ts = DB.isPostgreSQL() ? "COALESCE(I_ErrorMsg,'')" : "I_ErrorMsg";
    sql = (new StringBuffer("UPDATE I_Forecast f SET I_IsImported='E', I_ErrorMsg=" + ts + "||'ERR=Invalid Org, '" + "WHERE (AD_Org_ID IS NULL OR AD_Org_ID=0" + " OR EXISTS (SELECT * FROM AD_Org oo WHERE f.AD_Org_ID=oo.AD_Org_ID AND (oo.IsSummary='Y' OR oo.IsActive='N')))" + " AND I_IsImported<>'Y'")).append(clientCheck);
  //  no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(),  new Object[0], get_TrxName());
    
    if (no != 0)
      this.log.warning("Invalid Org=" + no); 
    sql = (new StringBuffer("UPDATE I_Forecast i SET MRP_Plan_ID=(SELECT MRP_Plan_ID FROM MRP_Plan P WHERE P.Name = i.Name2 AND P.IsActive='Y')WHERE MRP_Plan_ID IS NULL AND i.Name2 IS NOT NULL AND I_IsImported<>'Y'")).append(clientCheck);
    //no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    
    this.log.fine("Set Plan from Plan Name=" + no);
    sql = (new StringBuffer("UPDATE I_Forecast SET I_IsImported='E', I_ErrorMsg=" + ts + "||'ERR=Fill Mandatory - Plan, ' " + "WHERE MRP_Plan_ID IS NULL AND Name2 IS NULL" + " AND I_IsImported<>'Y'")).append(clientCheck);
    // no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =  DB.executeUpdateEx( sql.toString(), new Object[0], get_TrxName());
    
    if (no != 0)
      this.log.warning("Fill Mandatory - Plan=" + no); 
    sql = (new StringBuffer("UPDATE I_Forecast i SET Name2=(SELECT Name FROM MRP_Plan WHERE MRP_Plan_ID=i.MRP_Plan_ID) WHERE MRP_Plan_ID IS NOT NULL  AND I_IsImported<>'Y'")).append(clientCheck);
   // no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    
    this.log.fine("Update PlanName from Plan =" + no);
    sql = (new StringBuffer("UPDATE I_Forecast i SET I_IsImported='E', I_ErrorMsg=" + ts + "||'ERR=Enter a valid and active Plan Name (or) Fill Mandatory - Plan, ' " + "WHERE MRP_Plan_ID IS NULL AND Name2 IS NOT NULL AND NOT EXISTS(SELECT * FROM MRP_Plan WHERE MRP_Plan.Name = i.Name2 AND MRP_Plan.IsActive = 'Y') " + " AND I_IsImported<>'Y'")).append(clientCheck);
  //  no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    if (no != 0)
      this.log.warning("Enter a valid Plan Name=" + no); 
    sql = (new StringBuffer("UPDATE I_Forecast SET I_IsImported='E', I_ErrorMsg=" + ts + "||'ERR=Fill Mandatory - Name, ' " + "WHERE Name IS NULL" + " AND I_IsImported<>'Y'")).append(clientCheck);
   // no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    if (no != 0)
      this.log.warning("Fill Mandatory - Name=" + no); 
    sql = (new StringBuffer("UPDATE I_Forecast i SET M_Product_ID=(SELECT MAX(M_Product_ID) FROM M_Product p WHERE i.ProductValue=p.Value AND i.AD_Client_ID=p.AD_Client_ID) WHERE M_Product_ID IS NULL AND ProductValue IS NOT NULL AND I_IsImported<>'Y'")).append(clientCheck);
    //no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    
    this.log.fine("Set Product from ProductKey=" + no);
    sql = (new StringBuffer("UPDATE I_Forecast SET I_IsImported='E', I_ErrorMsg=" + ts + "||'ERR=Fill Mandatory - Product, ' " + "WHERE M_Product_ID IS NULL AND ProductValue IS NULL" + " AND I_IsImported<>'Y'")).append(clientCheck);
    //no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    if (no != 0)
      this.log.warning("Fill Mandatory - Product=" + no); 
    sql = (new StringBuffer("UPDATE I_Forecast i SET ProductValue = (SELECT Value FROM M_Product WHERE M_Product_ID=i.M_Product_ID)WHERE M_Product_ID IS NOT NULL  AND I_IsImported<>'Y'")).append(clientCheck);
    //no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    
    this.log.fine("Update ProductKey from Product=" + no);
    sql = (new StringBuffer("UPDATE I_Forecast SET I_IsImported='E', I_ErrorMsg=" + ts + "||'ERR=Enter a Valid ProductKey (or) Fill Mandatory - Product, ' " + "WHERE M_Product_ID IS NULL AND (ProductValue IS NOT NULL)" + " AND I_IsImported<>'Y'")).append(clientCheck);
   // no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    if (no != 0)
      this.log.warning("Invalid Product=" + no); 
    sql = (new StringBuffer("UPDATE I_Forecast i SET C_UOM_ID=(SELECT p.C_UOM_ID FROM M_Product p WHERE p.M_Product_ID=i.M_Product_ID) WHERE i.M_Product_ID IS NOT NULL AND (i.C_UOM_ID <> (SELECT p.C_UOM_ID FROM M_Product p WHERE p.M_Product_ID=i.M_Product_ID) OR C_UOM_ID IS NULL) AND i.I_IsImported<>'Y'")).append(clientCheck);
  //  no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    this.log.fine("Set UOM from Product=" + no);
    if (no != 0)
      this.log.warning("Invalid UOM, Updated UOM to product primary UOM=" + no); 
    sql = (new StringBuffer("UPDATE I_Forecast i SET Qty=(SELECT ROUND(i.Qty,u.StdPrecision) FROM C_UOM u WHERE ( i.C_UOM_ID = u.C_UOM_ID))WHERE i.I_IsImported<>'Y'")).append(clientCheck);
   // no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
   ...
   // no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    ...
    sql = (new StringBuffer("UPDATE I_Forecast SET I_IsImported='E', I_ErrorMsg=" + ts + "||'ERR=Fill Mandatory - Period, ' " + "WHERE C_Period_ID IS NULL AND PeriodName IS NULL " + " AND I_IsImported<>'Y'")).append(clientCheck);
   //no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    if (no != 0)
      this.log.warning("Fill Mandatory - Period=" + no); 
    sql = (new StringBuffer("UPDATE I_Forecast i SET PeriodName=(SELECT p.Name  FROM C_Period p WHERE p.C_Period_ID =  i.C_Period_ID AND p.C_Year_ID IN (SELECT C_Year_ID FROM C_Year WHERE C_Year.C_Calendar_ID = (SELECT C_Calendar_ID FROM MRP_Plan WHERE  MRP_Plan.MRP_Plan_ID = i.MRP_Plan_ID))) WHERE C_Period_ID IS NOT NULL AND MRP_Plan_ID IS NOT NULL AND I_IsImported<>'Y'")).append(clientCheck);
    //no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    ...
    int noUpdate = 0;
    int noUpdateLine = 0;
    sql = (new StringBuffer("SELECT * FROM I_Forecast WHERE I_IsImported='N'")).append(clientCheck).append(" ORDER BY MRP_Plan_ID, Name , M_Product_ID, C_Period_ID");
    ...
    try {
      ...
      boolean ForecastLineExists = false;
      String oldName = "";
      MMRPForecast forecast = null;
      MMRPForecastLine line = null;
      while (rs.next()) {
        X_I_Forecast imp = new X_I_Forecast(getCtx(), rs, get_TrxName());
        sql = (new StringBuffer("SELECT * FROM MRP_Forecast WHERE MRP_Plan_ID = " + imp.getMRP_Plan_ID() + " AND Name = '" + imp.getName() + "' ")).append(clientCheck).append(" ORDER BY MRP_Plan_ID, Name");
        ...
        if (rs1.next()) {
          ForecastExists = true;
          forecast = new MMRPForecast(getCtx(), rs1, get_TrxName());
          if (forecast.isFrozen()) {
            sql = (new StringBuffer("UPDATE I_Forecast SET I_IsImported='E', I_ErrorMsg='ERR=Cannot update forecast " + forecast.getName() + " which is frozen. ' " + "WHERE I_Forecast_ID = " + imp.getI_Forecast_ID())).append(clientCheck);
           // DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
            DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
            commit();
            this.log.warning("Cannot update forecast " + forecast.getName() + " which is frozen.");
            continue;
          } 
        } else {
          ForecastExists = false;
          forecast = new MMRPForecast(getCtx(), 0, get_TrxName());
        } 
        ...
        if (oldName == null)
          oldName = ""; 
        forecast.setClientOrg(imp.getAD_Client_ID(), imp.getAD_Org_ID());
        if (imp.getMRP_Plan_ID() != 0)
          forecast.setMRP_Plan_ID(imp.getMRP_Plan_ID()); 
        if (imp.getName() != null)
          forecast.setName(imp.getName()); 
        if (imp.getDescription() != null)
          forecast.setDescription(imp.getDescription()); 
        if (imp.getHelp() != null)
          forecast.setHelp(imp.getHelp()); 
        if (!ForecastExists) {
          forecast.setIsFrozen(false);
          forecast.setIsPreviouslyFrozen(false);
        } 
        if (!forecast.save()) {
          String msg = "Could not save Forecast";
          ValueNamePair pp = CLogger.retrieveError();
          if (pp != null)
            msg = msg + " - " + pp.toStringX(); 
          imp.setI_ErrorMsg(msg);
        ...
        imp.setMRP_Forecast_ID(forecast.getMRP_Forecast_ID());
        if (forecast.isPreviouslyFrozen()) {
          ...
          if (TimeUtil.getDay(System.currentTimeMillis()).after(period.getEndDate())) {
            sql = (new StringBuffer("UPDATE I_Forecast SET I_IsImported='E', I_ErrorMsg='ERR=Cannot update/insert forecast line for a previous period, Master Demand is created. ' WHERE I_Forecast_ID = " + imp.getI_Forecast_ID())).append(clientCheck);
          //  DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
            DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
           DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
            commit();
        ...
        if (ForecastExists == true) {
          sql = (new StringBuffer("SELECT * FROM MRP_ForecastLine WHERE M_Product_ID = " + imp.getM_Product_ID() + " AND C_Period_ID = " + imp.getC_Period_ID() + " AND MRP_Forecast_ID = " + imp.getMRP_Forecast_ID())).append(clientCheck).append(" ORDER BY M_Product_ID, C_Period_ID");
          ...
          if (rsLine.next()) {
            ForecastLineExists = true;
            line = new MMRPForecastLine(getCtx(), rsLine, get_TrxName());
            if (line.isLineFrozen()) {
              sql = (new StringBuffer("UPDATE I_Forecast SET I_IsImported='E', I_ErrorMsg='ERR=ForecastLine which is Frozen cannot be updated. ' WHERE I_Forecast_ID = " + imp.getI_Forecast_ID())).append(clientCheck);
             // DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
              DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
              commit();
              this.log.warning(forecast.getName() + ": ForecastLine which is Frozen cannot be updated.");
              continue;
            } 
          } else {
            ForecastLineExists = false;
            line = new MMRPForecastLine(forecast);
          } 
          rsLine.close();
        ...
        if (!line.save()) {
          String msg = "Could not save ForecastLine";
          ValueNamePair pp = CLogger.retrieveError();
          if (pp != null)
            msg = msg + " - " + pp.toStringX(); 
          imp.setI_ErrorMsg(msg);
        ...
        imp.setMRP_ForecastLine_ID(line.getMRP_ForecastLine_ID());
        imp.setI_IsImported(X_I_Forecast.I_ISIMPORTED_Yes);
        imp.setProcessed(true);
        if (imp.save())
          if (ForecastLineExists == true) {
            noUpdateLine++;
          } else {
            noInsertLine++;
          }  
      ...
      this.log.log(Level.SEVERE, "Forecast - " + sql.toString(), e);
    } 
    finally
    {
    ...
    sql = (new StringBuffer("UPDATE I_Forecast SET I_IsImported='N', Updated=SysDate WHERE I_IsImported<>'Y'")).append(clientCheck);
   // no = DB.executeUpdate(get_TrxName(), sql.toString(), new Object[0]);
    no =DB.executeUpdateEx(sql.toString(), new Object[0], get_TrxName());
    addLog(0, null, new BigDecimal(no), "@Errors@");
    addLog(0, null, new BigDecimal(noInsert), "@MRP_Forecast_ID@: @Inserted@");
    addLog(0, null, new BigDecimal(noInsertLine), "@MRP_ForecastLine_ID@: @Inserted@");
    addLog(0, null, new BigDecimal(noUpdate), "@MRP_Forecast_ID@: @Updated@");
    addLog(0, null, new BigDecimal(noUpdateLine), "@MRP_ForecastLine_ID@: @Updated@");
