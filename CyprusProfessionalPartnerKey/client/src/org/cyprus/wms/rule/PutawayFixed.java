...
public class PutawayFixed implements WMSRuleIntf {
  private static final CLogger s_log = CLogger.getCLogger(PutawayFixed.class);
  
  public MLocator[] getValidLocators(Properties ctx, int M_Warehouse_ID, int WMS_Zone_ID, int M_Product_ID, int C_OrderLine_ID, String trx) throws Exception {
    ...
    sql = sql + "AND l.M_Locator_ID IN (SELECT pl.M_Locator_ID FROM M_ProductLocator pl WHERE pl.M_Product_ID=? ) ";
    if (WMS_Zone_ID != 0)
    ...
    try {
      ...
      if (WMS_Zone_ID != 0)
        cPreparedStatement.setInt(index++, WMS_Zone_ID); 
