...
public class PutawayFloatingEmpty implements WMSRuleIntf {
  private static final CLogger s_log = CLogger.getCLogger(PutawayFloatingEmpty.class);
  ...
  public MLocator[] getValidLocators(Properties ctx, int M_Warehouse_ID, int M_Zone_ID, int M_Product_ID, int C_OrderLine_ID, String trx) throws Exception {
    ...
    sql = sql + "AND NOT EXISTS (SELECT 1 FROM M_StorageDetail s  WHERE s.M_Locator_ID=l.M_Locator_ID AND s.QtyType in ('H', 'E')  AND s.Qty > 0 ) ";
    if (M_Zone_ID != 0)
