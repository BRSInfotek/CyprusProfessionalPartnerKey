...
import org.cyprusbrs.framework.MAllocationHdr;
import org.cyprusbrs.framework.MAllocationLine;
import org.cyprusbrs.framework.MInvoice;
import org.cyprusbrs.framework.MPayment;
...
//	private int			i_multiplier = 10;
	...
	{		
		...
		StringBuffer sql = new StringBuffer("SELECT p.DateTrx,p.DocumentNo,p.C_Payment_ID,"  //  1..3
			+ "c.ISO_Code,"
//			+ "p.PayAmt,"                            //  4..5
			+ "case when p.IsPrepayment='Y' and p.IsTDSDeduction='Y' then p.PayAmtbeforeWithholding else p.PayAmt end as payamt,"
			+ "case when p.IsPrepayment='Y' and p.IsTDSDeduction='Y' then "
			+ "currencyConvert(p.PayAmtbeforeWithholding,p.C_Currency_ID,?,?,p.C_ConversionType_ID,p.AD_Client_ID,p.AD_Org_ID) "
			+ "else currencyConvert(p.PayAmt,p.C_Currency_ID,?,?,p.C_ConversionType_ID,p.AD_Client_ID,p.AD_Org_ID) end as payamount,"
			+ "case when p.IsPrepayment='Y' and p.IsTDSDeduction='Y' then "
			+ "currencyConvert(paymentavailableforwithholding(C_Payment_ID),p.C_Currency_ID,?,?,p.C_ConversionType_ID,p.AD_Client_ID,p.AD_Org_ID) "
			+ "else currencyConvert(paymentAvailable(C_Payment_ID),p.C_Currency_ID,?,?,p.C_ConversionType_ID,p.AD_Client_ID,p.AD_Org_ID) end as payavailableamount,"
