...
public class MFGExecutionAssembly extends GenForm
{
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(MFGExecutionAssembly.class);
	...
	public void configureMiniTable(IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(6, Integer.class, true,"M_Product_ID");
		miniTable.setColumnClass(7, Integer.class, true,"C_UOM_ID");
		//
		miniTable.autoSize();
	}
	...
	public void executeQuery(KeyNamePair docTypeKNPair, IMiniTable miniTable)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				miniTable.setValueAt((BigDecimal) m_Qty, row, 5);  
				miniTable.setValueAt(rs.getInt(3), row, 6);  
				miniTable.setValueAt(rs.getInt(5), row, 7); 
	...
	public String generateCostCollector(List<TimeRecordDTO> timeRecords)
	{
		...
		dataMap.put("CostCollectorType", "180");
